//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <FBSharedFramework/FBMPFilter-Protocol.h>

@class FBMPBaseConsumerPort, FBMPBaseProducerPort, IGContext, IGFilter, NSArray, NSString;

@interface FBBridgingIGFilter : NSObject <FBMPFilter>
{
    IGFilter *_filter;
    FBMPBaseConsumerPort *_videoConsumerPort;
    FBMPBaseProducerPort *_videoProducerPort;
    IGContext *_context;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) IGFilter *filter; // @synthesize filter=_filter;
- (void)invalidate;
- (void)sessionWillStop:(id)arg1;
- (void)sessionWillStart:(id)arg1;
- (void)sessionDisconnectedPort:(id)arg1 context:(id)arg2;
- (void)sessionConnectedPort:(id)arg1 context:(id)arg2;
@property(readonly, copy, nonatomic) NSArray *producerPorts;
- (id)consumerPorts;
- (void)_handleVideoData:(id)arg1 metadata:(struct FBMPDataMetadata)arg2;
- (id)initWithFilter:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

