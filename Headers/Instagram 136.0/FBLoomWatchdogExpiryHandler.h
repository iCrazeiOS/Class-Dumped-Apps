//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <FBSharedFramework/FBWatchdogExpiryHandling-Protocol.h>

@class FBWatchdogExpiryHandlerConfiguration, FBWatchdogStallFileManager, NSMutableDictionary, NSSet, NSString;

@interface FBLoomWatchdogExpiryHandler : NSObject <FBWatchdogExpiryHandling>
{
    FBWatchdogExpiryHandlerConfiguration *_configuration;
    FBWatchdogStallFileManager *_sharedStallFileManager;
    // Error parsing type: {atomic<double>="__a_"Ad}, name: _lastStallEndTime
    // Error parsing type: {atomic<bool>="__a_"AB}, name: _currentlyStalled
    // Error parsing type: {atomic<bool>="__a_"AB}, name: _traceInProgress
    // Error parsing type: {atomic<double>="__a_"Ad}, name: _maxTimeoutHit
    NSString *_lastTraceID;
    NSMutableDictionary *_watchdogExtraParams;
    shared_ptr_2811b282 _cppQPLLogger;
    NSSet *_timeouts;
}

@property(readonly, copy, nonatomic) NSSet *timeouts; // @synthesize timeouts=_timeouts;
- (id).cxx_construct;
- (void).cxx_destruct;
- (id)lastStallTraceID;
- (void)setExtraInfoString:(id)arg1 forKey:(id)arg2;
- (void)watchdogTimerWasStopped;
- (void)runLoopCompletedAtTime:(double)arg1;
- (void)runLoopTimedOutAtTime:(double)arg1 withTimeout:(double)arg2 atRunLoopActivity:(unsigned long long)arg3;
- (void)_setTraceID;
- (void)_writeWatchdogStallFile:(id)arg1 withTimeout:(double)arg2;
- (void)_abortTrace;
- (void)_endTrace;
- (void)_startTrace;
- (void)_calculateTimeouts;
- (id)initWithConfiguration:(id)arg1 logger:(shared_ptr_2811b282)arg2;

@end

