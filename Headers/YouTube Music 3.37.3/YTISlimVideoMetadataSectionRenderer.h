//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSData, NSMutableArray, NSString, YTICommand;

@interface YTISlimVideoMetadataSectionRenderer : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) NSMutableArray *contentsArray; // @dynamic contentsArray;
@property(readonly, nonatomic) unsigned long long contentsArray_Count; // @dynamic contentsArray_Count;
@property(nonatomic) _Bool hasLensVideoContinuationToken; // @dynamic hasLensVideoContinuationToken;
@property(nonatomic) _Bool hasSectionIdentifier; // @dynamic hasSectionIdentifier;
@property(nonatomic) _Bool hasTrackingParams; // @dynamic hasTrackingParams;
@property(nonatomic) _Bool hasUpdatedMetadataCommand; // @dynamic hasUpdatedMetadataCommand;
@property(nonatomic) _Bool hasVideoId; // @dynamic hasVideoId;
@property(copy, nonatomic) NSString *lensVideoContinuationToken; // @dynamic lensVideoContinuationToken;
@property(copy, nonatomic) NSString *sectionIdentifier; // @dynamic sectionIdentifier;
@property(copy, nonatomic) NSData *trackingParams; // @dynamic trackingParams;
@property(retain, nonatomic) YTICommand *updatedMetadataCommand; // @dynamic updatedMetadataCommand;
@property(copy, nonatomic) NSString *videoId; // @dynamic videoId;

@end

