//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "YTInlineReauthViewDelegate-Protocol.h"
#import "YTPageStyleProvider-Protocol.h"
#import "YTResponder-Protocol.h"

@class GIMMe, NSString, SSOIdentity, SSOProfileSource, SSOService, YTInlineReauthView;
@protocol SSOAuthorization, YTResponder;

@interface YTInlineReauthViewController : UIViewController <YTInlineReauthViewDelegate, YTPageStyleProvider, YTResponder>
{
    SSOIdentity *_identity;
    YTInlineReauthView *_reauthView;
    CDUnknownBlockType _successBlock;
    id <SSOAuthorization> _ssoAuthorization;
    SSOProfileSource *_ssoProfileSource;
    SSOService *_ssoService;
    id <YTResponder> _parentResponder;
    GIMMe *_gimme;
}

@property(nonatomic) __weak GIMMe *gimme; // @synthesize gimme=_gimme;
@property(readonly, nonatomic) __weak id <YTResponder> parentResponder; // @synthesize parentResponder=_parentResponder;
- (void).cxx_destruct;
- (void)fetchAvatarImage;
@property(readonly, nonatomic) long long pageStyle;
- (void)inlineReauthViewDidTapForgotPassword:(id)arg1;
- (void)inlineReauthView:(id)arg1 didTapNextWithPassword:(id)arg2;
- (double)headerHeightForInlineReauthView:(id)arg1;
- (unsigned long long)supportedInterfaceOrientations;
- (long long)preferredStatusBarStyle;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)loadView;
- (id)initWithAuthorization:(id)arg1 parentResponder:(id)arg2 successBlock:(CDUnknownBlockType)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

