//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <ChromeInternal/NSObject-Protocol.h>

@class NSArray, UIScrollView;
@protocol DSCScrollViewObserver;

@protocol DSCScrollableView <NSObject>
@property(readonly, nonatomic) NSArray *visibleCells;
@property(readonly, nonatomic) UIScrollView *backingView;
@property(readonly, nonatomic) long long scrollDirection;
@property(nonatomic) struct UIEdgeInsets contentInsets;
@property(nonatomic) struct CGPoint contentOffset;
@property(nonatomic) struct CGSize contentSize;
- (void)setContentOffset:(struct CGPoint)arg1 animated:(_Bool)arg2;
- (void)addSubviewToScrollView:(id)arg1;
- (void)removeScrollViewObserver:(id <DSCScrollViewObserver>)arg1;
- (void)addScrollViewObserver:(id <DSCScrollViewObserver>)arg1;
@end

