//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface NSObject (PerformHelper)
+ (id)yy_modelWithDictionary:(id)arg1;
+ (id)yy_modelWithJSON:(id)arg1;
+ (id)_yy_dictionaryWithJSON:(id)arg1;
- (void)dispatchBlockSyncOnMainThread:(CDUnknownBlockType)arg1;
- (void)performSelector:(SEL)arg1 withObject:(id)arg2 afterDelay:(double)arg3 cancelPrevious:(_Bool)arg4;
- (id)yy_modelDescription;
- (_Bool)yy_modelIsEqual:(id)arg1;
- (unsigned long long)yy_modelHash;
- (id)yy_modelInitWithCoder:(id)arg1;
- (void)yy_modelEncodeWithCoder:(id)arg1;
- (id)yy_modelCopy;
- (id)yy_modelToJSONString;
- (id)yy_modelToJSONData;
- (id)yy_modelToJSONObject;
- (_Bool)yy_modelSetWithDictionary:(id)arg1;
- (_Bool)yy_modelSetWithJSON:(id)arg1;
- (void)gtm_stopObservingAllKeyPaths;
- (void)gtm_removeObserver:(id)arg1 forKeyPath:(id)arg2 selector:(SEL)arg3;
- (void)gtm_addObserver:(id)arg1 forKeyPath:(id)arg2 selector:(SEL)arg3 userInfo:(id)arg4 options:(unsigned long long)arg5;
@end

