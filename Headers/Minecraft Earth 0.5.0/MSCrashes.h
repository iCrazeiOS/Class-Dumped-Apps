...
@class MSChannelUnitConfiguration, MSEncrypter, MSErrorReport, MSPLCrashReporter, NSMutableArray, NSObject<OS_dispatch_group>, NSObject<OS_dispatch_queue>, NSObject<OS_dispatch_semaphore>, NSString;

@interface MSCrashes : MSServiceAbstract <MSChannelDelegate, MSServiceInternal>
{
    _Bool _didCrashInLastSession;
    _Bool _shouldReleaseProcessingSemaphore;
    _Bool _enableMachExceptionHandler;
    _Bool _automaticProcessingEnabled;
...
+ (_Bool)validatePropertiesForAttachment:(id)arg1;
+ (id)logTag;
+ (id)serviceName;
+ (id)sharedInstance;
...
+ (_Bool)hasCrashedInLastSession;
+ (void)generateTestCrash;
@property(nonatomic, getter=isAutomaticProcessingEnabled) _Bool automaticProcessingEnabled; // @synthesize automaticProcessingEnabled=_automaticProcessingEnabled;
...
@property _Bool shouldReleaseProcessingSemaphore; // @synthesize shouldReleaseProcessingSemaphore=_shouldReleaseProcessingSemaphore;
@property _Bool didCrashInLastSession; // @synthesize didCrashInLastSession=_didCrashInLastSession;
...
- (void)notifyWithUserConfirmation:(unsigned long long)arg1;
- (_Bool)shouldAlwaysSend;
- (_Bool)userPromptedForConfirmation;
- (_Bool)sendCrashReportsOrAwaitUserConfirmation;
- (_Bool)shouldProcessErrorReport:(id)arg1;
...
- (void)configureCrashReporterWithUncaughtExceptionHandlerEnabled:(_Bool)arg1;
