//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Funny/DDLogFileManagerDefault.h>

@class NSObject;
@protocol OS_dispatch_queue;

@interface FNLogFileManager : DDLogFileManagerDefault
{
    _Bool _isUploading;
    NSObject<OS_dispatch_queue> *_uploadQueue;
}

- (void).cxx_destruct;
@property(nonatomic) _Bool isUploading; // @synthesize isUploading=_isUploading;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *uploadQueue; // @synthesize uploadQueue=_uploadQueue;
- (void)uploadLogFilesToServerIfNeededWithCompletion:(CDUnknownBlockType)arg1;
- (void)didRollAndArchiveLogFile:(id)arg1;
- (id)init;

@end

