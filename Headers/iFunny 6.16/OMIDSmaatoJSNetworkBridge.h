//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@protocol OMIDSmaatoJSInvoker;

@interface OMIDSmaatoJSNetworkBridge : NSObject
{
    id <OMIDSmaatoJSInvoker> _jsInvoker;
}

@property(nonatomic) __weak id <OMIDSmaatoJSInvoker> jsInvoker; // @synthesize jsInvoker=_jsInvoker;
- (void).cxx_destruct;
- (unsigned long long)getOMIDError:(id)arg1;
- (void)injectJavaScriptFromURL:(id)arg1;
- (void)sendURL:(id)arg1 successCallback:(id)arg2 failureCallback:(id)arg3;
- (void)downloadJavaScriptResourceWithURL:(id)arg1 successCallback:(id)arg2 failureCallback:(id)arg3;
- (void)setupMethodsForJSObject:(id)arg1;

@end

