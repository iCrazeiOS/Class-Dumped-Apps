//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

#import "ASTFormField-Protocol.h"
#import "ASTThemable-Protocol.h"

@class GOOActionSheetController, GPBMessage, MDCButton, NSString, UILabel;
@protocol ASTFormRendererDelegate;

@interface ASTFormList : UIView <ASTThemable, ASTFormField>
{
    id <ASTFormRendererDelegate> _fieldDelegate;
    GPBMessage *_proto;
    NSString *_fieldID;
    NSString *_intentFieldID;
    UILabel *_listNameLabel;
    MDCButton *_listNameButton;
    GOOActionSheetController *_actionController;
}

@property(retain, nonatomic) GOOActionSheetController *actionController; // @synthesize actionController=_actionController;
@property(retain, nonatomic) MDCButton *listNameButton; // @synthesize listNameButton=_listNameButton;
@property(retain, nonatomic) UILabel *listNameLabel; // @synthesize listNameLabel=_listNameLabel;
@property(retain, nonatomic) NSString *intentFieldID; // @synthesize intentFieldID=_intentFieldID;
@property(retain, nonatomic) NSString *fieldID; // @synthesize fieldID=_fieldID;
@property(retain, nonatomic) GPBMessage *proto; // @synthesize proto=_proto;
@property(nonatomic) __weak id <ASTFormRendererDelegate> fieldDelegate; // @synthesize fieldDelegate=_fieldDelegate;
- (void).cxx_destruct;
- (void)showList;
- (void)setupEvents;
- (void)setupConstraints;
- (void)setupViews;
- (void)applyTheme:(unsigned long long)arg1;
- (id)initWithFieldID:(id)arg1 intentID:(id)arg2 delegate:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

