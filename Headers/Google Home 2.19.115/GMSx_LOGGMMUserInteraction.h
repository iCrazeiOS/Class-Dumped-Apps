//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GMSx_GPBMessage.h"

@class GMSx_LOGGMMVisualElement, GMSx_LOGMapsInteractionData, GMSx_LOGUIState, GMSx_LOGVisualElementLiteProto, NSMutableArray, NSString;

@interface GMSx_LOGGMMUserInteraction : GMSx_GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) int action; // @dynamic action;
@property(nonatomic) int cardinalDirection; // @dynamic cardinalDirection;
@property(retain, nonatomic) GMSx_LOGVisualElementLiteProto *clientLeafVe; // @dynamic clientLeafVe;
@property(retain, nonatomic) NSMutableArray *clientVeTreeArray; // @dynamic clientVeTreeArray;
@property(readonly, nonatomic) unsigned long long clientVeTreeArray_Count; // @dynamic clientVeTreeArray_Count;
@property(copy, nonatomic) NSString *contentEi; // @dynamic contentEi;
@property(retain, nonatomic) GMSx_LOGGMMVisualElement *contentVe; // @dynamic contentVe;
@property(nonatomic) long long featureFprint; // @dynamic featureFprint;
@property(nonatomic) _Bool hasAction; // @dynamic hasAction;
@property(nonatomic) _Bool hasCardinalDirection; // @dynamic hasCardinalDirection;
@property(nonatomic) _Bool hasClientLeafVe; // @dynamic hasClientLeafVe;
@property(nonatomic) _Bool hasContentEi; // @dynamic hasContentEi;
@property(nonatomic) _Bool hasContentVe; // @dynamic hasContentVe;
@property(nonatomic) _Bool hasFeatureFprint; // @dynamic hasFeatureFprint;
@property(nonatomic) _Bool hasIsFeatureFprintScrubbed; // @dynamic hasIsFeatureFprintScrubbed;
@property(nonatomic) _Bool hasParams; // @dynamic hasParams;
@property(nonatomic) _Bool hasPrimaryUserAction; // @dynamic hasPrimaryUserAction;
@property(nonatomic) _Bool hasUiState; // @dynamic hasUiState;
@property(nonatomic) _Bool isFeatureFprintScrubbed; // @dynamic isFeatureFprintScrubbed;
@property(retain, nonatomic) GMSx_LOGMapsInteractionData *params; // @dynamic params;
@property(nonatomic) int primaryUserAction; // @dynamic primaryUserAction;
@property(retain, nonatomic) GMSx_LOGUIState *uiState; // @dynamic uiState;

@end

