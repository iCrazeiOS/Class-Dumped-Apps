//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class GCADImage, NSString;

@interface GCADPageHeader : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(copy, nonatomic) NSString *actionLink; // @dynamic actionLink;
@property(copy, nonatomic) NSString *actionText; // @dynamic actionText;
@property(nonatomic) _Bool hasActionLink; // @dynamic hasActionLink;
@property(nonatomic) _Bool hasActionText; // @dynamic hasActionText;
@property(nonatomic) _Bool hasImage; // @dynamic hasImage;
@property(nonatomic) _Bool hasPartnerIcon; // @dynamic hasPartnerIcon;
@property(nonatomic) _Bool hasTitle; // @dynamic hasTitle;
@property(retain, nonatomic) GCADImage *image; // @dynamic image;
@property(retain, nonatomic) GCADImage *partnerIcon; // @dynamic partnerIcon;
@property(copy, nonatomic) NSString *title; // @dynamic title;

@end

