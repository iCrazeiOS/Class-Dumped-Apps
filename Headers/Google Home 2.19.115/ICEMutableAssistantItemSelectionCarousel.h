//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "ICEAssistantItemSelectionCarousel.h"

#import "ICEMutableAssistantItemSelectionCarousel-Protocol.h"

@class ICEAccessibility, ICEBehavior, ICEStyle, ICEVisualElementLogInfo, NSArray, NSString;

@interface ICEMutableAssistantItemSelectionCarousel : ICEAssistantItemSelectionCarousel <ICEMutableAssistantItemSelectionCarousel>
{
}


// Remaining properties
@property(copy, nonatomic) NSString *UID;
@property(copy, nonatomic) ICEAccessibility *accessibility;
@property(copy, nonatomic) ICEBehavior *behavior;
@property(copy, nonatomic) NSArray *contents; // @dynamic contents;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(copy, nonatomic) NSArray *layoutConstraints;
@property(copy, nonatomic) NSArray *layoutGuideUIDs;
@property(copy, nonatomic) ICEVisualElementLogInfo *logInfo;
@property(nonatomic) unsigned long long selectedItemIndex; // @dynamic selectedItemIndex;
@property(nonatomic) long long state; // @dynamic state;
@property(copy, nonatomic) ICEStyle *style;
@property(readonly) Class superclass;
@end

