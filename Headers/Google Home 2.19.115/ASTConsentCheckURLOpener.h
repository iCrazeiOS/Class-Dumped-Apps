//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "ASTSettingsURLOpener-Protocol.h"

@class ASTSSOManager, NSString, ULRManager;

@interface ASTConsentCheckURLOpener : NSObject <ASTSettingsURLOpener>
{
    ASTSSOManager *_ssoManager;
    ULRManager *_ulrManager;
}

- (void).cxx_destruct;
- (void)checkUDCConsentWithPresentingViewController:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (_Bool)openURL:(id)arg1 completion:(CDUnknownBlockType)arg2 presentingViewController:(id)arg3;
- (id)initWithSSOManager:(id)arg1 ULRManager:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

