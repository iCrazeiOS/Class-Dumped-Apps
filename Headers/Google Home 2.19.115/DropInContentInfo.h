//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface DropInContentInfo : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(copy, nonatomic) NSString *deviceModelType; // @dynamic deviceModelType;
@property(nonatomic) long long duration; // @dynamic duration;
@property(copy, nonatomic) NSString *eventId; // @dynamic eventId;
@property(nonatomic) int eventType; // @dynamic eventType;
@property(nonatomic) _Bool hasDeviceModelType; // @dynamic hasDeviceModelType;
@property(nonatomic) _Bool hasDuration; // @dynamic hasDuration;
@property(nonatomic) _Bool hasEventId; // @dynamic hasEventId;
@property(nonatomic) _Bool hasEventType; // @dynamic hasEventType;
@property(nonatomic) _Bool hasMinutesAfterEvent; // @dynamic hasMinutesAfterEvent;
@property(nonatomic) int minutesAfterEvent; // @dynamic minutesAfterEvent;

@end

