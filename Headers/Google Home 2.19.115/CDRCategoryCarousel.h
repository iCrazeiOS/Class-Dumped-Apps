//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSMutableArray, NSString;

@interface CDRCategoryCarousel : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) NSMutableArray *cardsArray; // @dynamic cardsArray;
@property(readonly, nonatomic) unsigned long long cardsArray_Count; // @dynamic cardsArray_Count;
@property(nonatomic) _Bool hasMoreButtonText; // @dynamic hasMoreButtonText;
@property(nonatomic) _Bool hasMoreButtonURL; // @dynamic hasMoreButtonURL;
@property(copy, nonatomic) NSString *moreButtonText; // @dynamic moreButtonText;
@property(copy, nonatomic) NSString *moreButtonURL; // @dynamic moreButtonURL;

@end

