//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "ComGoogleCommonCollectImmutableSet_Indexed.h"

@protocol ComGoogleCommonCollectMultiset, JavaUtilList;

@interface ComGoogleCommonCollectImmutableMultiset_ElementSet : ComGoogleCommonCollectImmutableSet_Indexed
{
    id <JavaUtilList> entries_;
    id <ComGoogleCommonCollectMultiset> delegate_;
}

- (void)dealloc;
- (unsigned long long)countByEnumeratingWithState:(CDStruct_58648341 *)arg1 objects:(id *)arg2 count:(unsigned long long)arg3;
- (int)size;
- (_Bool)isPartialView;
- (_Bool)containsWithId:(id)arg1;
- (id)getWithInt:(int)arg1;
- (id)initWithJavaUtilList:(id)arg1 withComGoogleCommonCollectMultiset:(id)arg2;

@end

