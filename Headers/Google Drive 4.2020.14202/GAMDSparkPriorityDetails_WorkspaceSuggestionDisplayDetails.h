//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <drive_extension_framework/GPBMessage.h>

@interface GAMDSparkPriorityDetails_WorkspaceSuggestionDisplayDetails : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) int currentWorkspaceCount; // @dynamic currentWorkspaceCount;
@property(nonatomic) _Bool hasCurrentWorkspaceCount; // @dynamic hasCurrentWorkspaceCount;
@property(nonatomic) _Bool hasPredictionIndex; // @dynamic hasPredictionIndex;
@property(nonatomic) _Bool hasSuggestionSource; // @dynamic hasSuggestionSource;
@property(nonatomic) int predictionIndex; // @dynamic predictionIndex;
@property(nonatomic) int suggestionSource; // @dynamic suggestionSource;

@end

