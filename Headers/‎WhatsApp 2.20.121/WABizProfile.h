//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSString, WABizHours, WABizProfileOptions, WABizVertical, WABusinessStructuredAddress;

@interface WABizProfile : NSObject
{
    long long _tag;
    NSString *_address;
    double _latitude;
    double _longitude;
    WABusinessStructuredAddress *_structuredAddress;
    NSString *_businessDescription;
    WABizHours *_hours;
    WABizVertical *_vertical;
    NSArray *_categories;
    NSString *_email;
    NSArray *_websites;
    NSArray *_websitesExcludingSocial;
    NSArray *_socialProfiles;
    long long _flags;
    WABizProfileOptions *_profileOptions;
}

+ (long long)version;
- (void).cxx_destruct;
@property(readonly, nonatomic) WABizProfileOptions *profileOptions; // @synthesize profileOptions=_profileOptions;
@property(nonatomic) long long flags; // @synthesize flags=_flags;
@property(readonly, nonatomic) NSArray *socialProfiles; // @synthesize socialProfiles=_socialProfiles;
@property(readonly, nonatomic) NSArray *websitesExcludingSocial; // @synthesize websitesExcludingSocial=_websitesExcludingSocial;
@property(readonly, nonatomic) NSArray *websites; // @synthesize websites=_websites;
@property(copy, nonatomic) NSString *email; // @synthesize email=_email;
@property(copy, nonatomic) NSArray *categories; // @synthesize categories=_categories;
@property(copy, nonatomic) WABizVertical *vertical; // @synthesize vertical=_vertical;
@property(copy, nonatomic) WABizHours *hours; // @synthesize hours=_hours;
@property(copy, nonatomic) NSString *businessDescription; // @synthesize businessDescription=_businessDescription;
@property(readonly, nonatomic) WABusinessStructuredAddress *structuredAddress; // @synthesize structuredAddress=_structuredAddress;
@property(readonly, nonatomic) double longitude; // @synthesize longitude=_longitude;
@property(readonly, nonatomic) double latitude; // @synthesize latitude=_latitude;
@property(copy, nonatomic) NSString *address; // @synthesize address=_address;
@property(nonatomic) long long tag; // @synthesize tag=_tag;
- (_Bool)isEqual:(id)arg1;
- (_Bool)isEqualToBizProfile:(id)arg1;
- (unsigned long long)hash;
@property(readonly, copy, nonatomic) NSString *detailsInfoString;
- (id)infoString;
- (_Bool)isEmpty;
- (_Bool)hasProductCatalog;
- (_Bool)hasBizInfo;
- (_Bool)hasAddress;
- (id)addressDisplayTitle;
- (_Bool)hasValidCoordinates;
- (id)initWithProfileData:(id)arg1;

@end

