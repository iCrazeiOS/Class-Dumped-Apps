//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <T1Twitter/TLPSubscriptionDelegate-Protocol.h>
#import <T1Twitter/TNLLRUEntry-Protocol.h>

@class NSString, T1StatusLiveEngagementManager, TFNTwitterStatus, TFSTwitterStatusLivePipelineUpdateHandle, TLPSubscription;
@protocol TNLLRUEntry;

@interface TFNLivePipelineTweetEntry : NSObject <TNLLRUEntry, TLPSubscriptionDelegate>
{
    id <TNLLRUEntry> _nextLRUEntry;
    id <TNLLRUEntry> _previousLRUEntry;
    T1StatusLiveEngagementManager *_tweetManager;
    TFNTwitterStatus *_status;
    TLPSubscription *_subscription;
    TFSTwitterStatusLivePipelineUpdateHandle *_statusLivePipelineUpdate;
}

+ (id)identifierForStatus:(id)arg1;
@property(retain, nonatomic) TFSTwitterStatusLivePipelineUpdateHandle *statusLivePipelineUpdate; // @synthesize statusLivePipelineUpdate=_statusLivePipelineUpdate;
@property(retain, nonatomic) TLPSubscription *subscription; // @synthesize subscription=_subscription;
@property(retain, nonatomic) TFNTwitterStatus *status; // @synthesize status=_status;
@property(nonatomic) __weak T1StatusLiveEngagementManager *tweetManager; // @synthesize tweetManager=_tweetManager;
@property(nonatomic) __weak id <TNLLRUEntry> previousLRUEntry; // @synthesize previousLRUEntry=_previousLRUEntry;
@property(retain, nonatomic) id <TNLLRUEntry> nextLRUEntry; // @synthesize nextLRUEntry=_nextLRUEntry;
- (void).cxx_destruct;
- (void)subscription:(id)arg1 didReceiveEvent:(id)arg2;
- (_Bool)shouldAccessMoveLRUEntryToHead;
- (id)LRUEntryIdentifier;
- (id)initWithStatus:(id)arg1 tweetManager:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

