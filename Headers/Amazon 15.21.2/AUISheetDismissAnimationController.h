//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "UIViewControllerAnimatedTransitioning-Protocol.h"

@class AUISheetViewController, NSString, UIView, UIViewController;

@interface AUISheetDismissAnimationController : NSObject <UIViewControllerAnimatedTransitioning>
{
    AUISheetViewController *_sheetViewController;
    UIView *_lightBoxView;
    UIView *_screenShotView;
    UIViewController *_destinationViewController;
    double _duration;
    double _height;
}

@property(nonatomic) double height; // @synthesize height=_height;
@property(nonatomic) double duration; // @synthesize duration=_duration;
@property(retain, nonatomic) UIViewController *destinationViewController; // @synthesize destinationViewController=_destinationViewController;
@property(retain, nonatomic) UIView *screenShotView; // @synthesize screenShotView=_screenShotView;
@property(retain, nonatomic) UIView *lightBoxView; // @synthesize lightBoxView=_lightBoxView;
@property(retain, nonatomic) AUISheetViewController *sheetViewController; // @synthesize sheetViewController=_sheetViewController;
- (void).cxx_destruct;
- (void)animateTransition:(id)arg1;
- (double)transitionDuration:(id)arg1;
- (id)initWithSheetHeight:(double)arg1 duration:(double)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

