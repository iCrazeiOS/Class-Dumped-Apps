//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "_TtC5Slack27SearchModulesViewController.h"

@interface _TtC5Slack27SearchModulesViewController (Slack6)
- (id)autocompletePresentationViewWithSearchBarView:(id)arg1;
- (_Bool)searchBarShouldClear:(id)arg1;
- (void)searchBarDidEditText:(id)arg1 text:(id)arg2;
- (_Bool)searchBar:(id)arg1 shouldChangeTextInRange:(struct _NSRange)arg2 replacementText:(id)arg3;
- (void)searchBarDidReturn:(id)arg1;
- (void)searchBarDidCancel:(id)arg1;
- (void)searchBarDidEndEditing:(id)arg1;
- (void)searchBarDidBeginEditing:(id)arg1;
- (_Bool)searchBarShouldBeginEditing:(id)arg1;
@end

