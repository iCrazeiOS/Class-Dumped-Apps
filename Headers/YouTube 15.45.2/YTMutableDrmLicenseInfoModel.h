//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Module_Framework/YTDrmLicenseInfoModel.h>

@class NSData, NSString;

@interface YTMutableDrmLicenseInfoModel : YTDrmLicenseInfoModel
{
}

- (void)parseAndExtractFieldsFromProtoIfNecessary;
- (id)copyWithZone:(struct _NSZone *)arg1;
@property(copy, nonatomic) NSData *data; // @dynamic data;
@property(retain, nonatomic) NSData *fairPlayConfig; // @dynamic fairPlayConfig;
@property(retain, nonatomic) NSString *drmSessionId; // @dynamic drmSessionId;
@property(retain, nonatomic) NSString *drmParams; // @dynamic drmParams;
@property(nonatomic) _Bool canRenew; // @dynamic canRenew;
@property(nonatomic) long long playbackDurationSeconds; // @dynamic playbackDurationSeconds;
@property(nonatomic) long long licenseDurationSeconds; // @dynamic licenseDurationSeconds;
@property(retain, nonatomic) NSString *identifier; // @dynamic identifier;
- (id)initWithEntityStore:(id)arg1;

@end

