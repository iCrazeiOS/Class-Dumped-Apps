//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Module_Framework/GPBMessage.h>

@class NSMutableArray, NSString, YTIRenderer;

@interface YTICreatePlaylistEndpoint : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) YTIRenderer *createPlaylistDialog; // @dynamic createPlaylistDialog;
@property(nonatomic) _Bool hack; // @dynamic hack;
@property(nonatomic) _Bool hasCreatePlaylistDialog; // @dynamic hasCreatePlaylistDialog;
@property(nonatomic) _Bool hasHack; // @dynamic hasHack;
@property(nonatomic) _Bool hasParams; // @dynamic hasParams;
@property(nonatomic) _Bool hasSelectionSessionEntityKey; // @dynamic hasSelectionSessionEntityKey;
@property(nonatomic) _Bool hasSourcePlaylistId; // @dynamic hasSourcePlaylistId;
@property(copy, nonatomic) NSString *params; // @dynamic params;
@property(copy, nonatomic) NSString *selectionSessionEntityKey; // @dynamic selectionSessionEntityKey;
@property(copy, nonatomic) NSString *sourcePlaylistId; // @dynamic sourcePlaylistId;
@property(retain, nonatomic) NSMutableArray *videoIdsArray; // @dynamic videoIdsArray;
@property(readonly, nonatomic) unsigned long long videoIdsArray_Count; // @dynamic videoIdsArray_Count;

@end

