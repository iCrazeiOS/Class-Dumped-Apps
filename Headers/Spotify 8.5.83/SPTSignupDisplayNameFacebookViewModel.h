//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SPTSignupDataLoaderDelegate-Protocol.h"
#import "SPTSignupDisplayNameViewModelProtocol-Protocol.h"

@class NSString, SPTAuthenticationHandler, SPTLoginDialogController, SPTLoginEmailValidationDialogLogger, SPTLoginErrorDialogLogger, SPTLoginSplitEmailSignupViewLogger, SPTSignupCreateUserDataLoader, SPTSignupTermsAndPolicyViewModel, SPTSignupUserInfoModel;
@protocol SPTLoginNavigationCoordinator, SPTSignupDisplayNameViewModelDelegate;

@interface SPTSignupDisplayNameFacebookViewModel : NSObject <SPTSignupDataLoaderDelegate, SPTSignupDisplayNameViewModelProtocol>
{
    id <SPTSignupDisplayNameViewModelDelegate> _delegate;
    SPTLoginSplitEmailSignupViewLogger *_logger;
    SPTSignupTermsAndPolicyViewModel *_termsAndPolicyViewModel;
    SPTSignupUserInfoModel *_userInfoModel;
    SPTSignupCreateUserDataLoader *_createUserDataLoader;
    SPTLoginErrorDialogLogger *_dialogLogger;
    SPTAuthenticationHandler *_authenticationHandler;
    SPTLoginDialogController *_dialogController;
    id <SPTLoginNavigationCoordinator> _navigationCoordinator;
    SPTLoginEmailValidationDialogLogger *_emailExistsDialogLogger;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) SPTLoginEmailValidationDialogLogger *emailExistsDialogLogger; // @synthesize emailExistsDialogLogger=_emailExistsDialogLogger;
@property(readonly, nonatomic) id <SPTLoginNavigationCoordinator> navigationCoordinator; // @synthesize navigationCoordinator=_navigationCoordinator;
@property(readonly, nonatomic) SPTLoginDialogController *dialogController; // @synthesize dialogController=_dialogController;
@property(readonly, nonatomic) SPTAuthenticationHandler *authenticationHandler; // @synthesize authenticationHandler=_authenticationHandler;
@property(readonly, nonatomic) SPTLoginErrorDialogLogger *dialogLogger; // @synthesize dialogLogger=_dialogLogger;
@property(readonly, nonatomic) SPTSignupCreateUserDataLoader *createUserDataLoader; // @synthesize createUserDataLoader=_createUserDataLoader;
@property(retain, nonatomic) SPTSignupUserInfoModel *userInfoModel; // @synthesize userInfoModel=_userInfoModel;
@property(readonly, nonatomic) SPTSignupTermsAndPolicyViewModel *termsAndPolicyViewModel; // @synthesize termsAndPolicyViewModel=_termsAndPolicyViewModel;
@property(readonly, nonatomic) SPTLoginSplitEmailSignupViewLogger *logger; // @synthesize logger=_logger;
@property(nonatomic) __weak id <SPTSignupDisplayNameViewModelDelegate> delegate; // @synthesize delegate=_delegate;
- (void)presentLoginViewWithEmail:(id)arg1;
- (void)dataLoader:(id)arg1 didFinishLoadingWithError:(id)arg2;
- (void)dataLoader:(id)arg1 didFinishLoadingWithSuccessResponse:(id)arg2;
- (void)createUserAndPresentMainScreen;
- (void)textFieldUpdated:(id)arg1;
- (void)setDisplayName:(id)arg1;
- (_Bool)isSouthKorea;
- (id)requiredLabelText;
- (id)displayNameSuggestion;
- (id)createButtonLoadingText;
- (id)createButtonText;
- (id)displayNameDisclosureText;
- (id)displayNameLabelText;
- (id)titleLabelText;
- (id)initWithUserInfoModel:(id)arg1 createUserDataLoader:(id)arg2 logger:(id)arg3 dialogLogger:(id)arg4 termsAndPolicyViewModel:(id)arg5 authenticationHandler:(id)arg6 dialogController:(id)arg7 navigationCoordinator:(id)arg8 emailExistsDialogLogger:(id)arg9;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

