//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface _TtC27ListeningHistoryFeatureImpl23ListeningHistoryService : NSObject
{
    // Error parsing type: , name: viewURI
    // Error parsing type: , name: pagedContentOperationObserver
    // Error parsing type: , name: featureProperties
    // Error parsing type: , name: _containerService
    // Error parsing type: , name: _remoteConfigurationService
    // Error parsing type: , name: _snackbarService
    // Error parsing type: , name: _networkService
    // Error parsing type: , name: _glueService
    // Error parsing type: , name: _encoreIntegrationService
    // Error parsing type: , name: _playerService
    // Error parsing type: , name: _hubFrameworkService
    // Error parsing type: , name: _hubSwiftService
    // Error parsing type: , name: _freeTierUIService
    // Error parsing type: , name: _homeUIService
    // Error parsing type: , name: _sortingFilteringService
    // Error parsing type: , name: _clientSessionService
    // Error parsing type: , name: _ubiService
    // Error parsing type: , name: $__lazy_storage_$_snackbarPresenter
    // Error parsing type: , name: $__lazy_storage_$_player
    // Error parsing type: , name: $__lazy_storage_$_pagedContentOperationObserverManager
    // Error parsing type: , name: $__lazy_storage_$_viewControllerFactory
}

+ (id)serviceIdentifier;
- (void).cxx_destruct;
- (id)init;
- (void)unload;
- (void)load;
- (id)provideListeningHistoryLogger;

@end

