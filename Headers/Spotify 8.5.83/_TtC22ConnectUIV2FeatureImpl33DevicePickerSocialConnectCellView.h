//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UITableViewCell.h>

@interface _TtC22ConnectUIV2FeatureImpl33DevicePickerSocialConnectCellView : UITableViewCell
{
    // Error parsing type: , name: theme
    // Error parsing type: , name: imageFactory
    // Error parsing type: , name: imageLoader
    // Error parsing type: , name: $__lazy_storage_$_contentContainer
    // Error parsing type: , name: $__lazy_storage_$_secondaryViewContainer
    // Error parsing type: , name: $__lazy_storage_$_facepileContainerView
    // Error parsing type: , name: facepileView
    // Error parsing type: , name: $__lazy_storage_$_mainLabel
    // Error parsing type: , name: $__lazy_storage_$_secondaryIconImageView
    // Error parsing type: , name: $__lazy_storage_$_secondaryLabel
    // Error parsing type: , name: $__lazy_storage_$_joinButton
    // Error parsing type: , name: $__lazy_storage_$_contextMenuButton
    // Error parsing type: , name: isDisabled
    // Error parsing type: , name: leaveButtonToContextMenuButtonConstraint
    // Error parsing type: , name: contextMenuWidthConstraint
    // Error parsing type: , name: mainLabelToSecondaryViewConstraint
    // Error parsing type: , name: secondaryViewHeightConstraint
    // Error parsing type: , name: secondaryIconViewToSecondaryLabelConstraint
    // Error parsing type: , name: secondaryIconWidthConstraint
    // Error parsing type: , name: secondaryIcon
    // Error parsing type: , name: participants
}

- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

