//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "_TtC3QVC28CheckoutBaseMarginModuleCell.h"

@class UIImageView, UILabel, UIView;

@interface _TtC3QVC24QVCAddressListModuleCell : _TtC3QVC28CheckoutBaseMarginModuleCell
{
    // Error parsing type: , name: placeholder
    // Error parsing type: , name: titleLabel
    // Error parsing type: , name: arrowImage
    // Error parsing type: , name: border
    // Error parsing type: , name: didShowActionSheetEvent
}

- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;
- (void)didShowActionSheetAction:(id)arg1;
@property(nonatomic) __weak UIView *border; // @synthesize border;
@property(nonatomic) __weak UIImageView *arrowImage; // @synthesize arrowImage;
@property(nonatomic) __weak UILabel *titleLabel; // @synthesize titleLabel;
@property(nonatomic) __weak UILabel *placeholder; // @synthesize placeholder;

@end

