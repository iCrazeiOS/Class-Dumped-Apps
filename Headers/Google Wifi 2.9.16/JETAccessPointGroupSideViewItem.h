//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GOOSideViewItem.h"

#import "JETSideViewItem-Protocol.h"

@class GOOSideViewItemLayoutSource, JETAccessPointGroup, NSNumber, NSString, OGLListItemControlStateStyling, UIColor, UIImage, UIView;
@protocol JETUserStateProtocol;

@interface JETAccessPointGroupSideViewItem : GOOSideViewItem <JETSideViewItem>
{
    _Bool _selected;
    id <JETUserStateProtocol> _userState;
    JETAccessPointGroup *_group;
}

@property(nonatomic, getter=isSelected) _Bool selected; // @synthesize selected=_selected;
@property(retain, nonatomic) JETAccessPointGroup *group; // @synthesize group=_group;
@property(nonatomic) __weak id <JETUserStateProtocol> userState; // @synthesize userState=_userState;
- (void).cxx_destruct;
- (void)selectedFromPanelViewController:(id)arg1;
@property(readonly, nonatomic) UIColor *textColor;
- (id)initWithTitle:(id)arg1 icon:(id)arg2 selectedIcon:(id)arg3;
- (id)initWithUserState:(id)arg1 group:(id)arg2;

// Remaining properties
@property(readonly, copy, nonatomic) NSString *accessibilityHint;
@property(readonly, copy, nonatomic) NSString *accessibilityIdentifier;
@property(readonly, copy, nonatomic) NSString *accessibilityLabel;
@property(readonly, nonatomic) UIView *accessoryView;
@property(readonly, nonatomic) UIView *backgroundView;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly, nonatomic) _Bool disableIconTint;
@property(readonly, nonatomic) _Bool disabled;
@property(readonly, nonatomic) OGLListItemControlStateStyling *disabledStateStyling;
@property(readonly) unsigned long long hash;
@property(readonly, nonatomic) OGLListItemControlStateStyling *highlightedSelectedStateStyling;
@property(readonly, nonatomic) OGLListItemControlStateStyling *highlightedStateStyling;
@property(readonly, nonatomic) UIImage *icon;
@property(readonly, nonatomic) UIColor *iconColor;
@property(readonly, nonatomic) GOOSideViewItemLayoutSource *layoutSource;
@property(readonly, nonatomic) NSNumber *lineBreakMode;
@property(nonatomic, setter=mdc_setAdjustsFontForContentSizeCategory:) _Bool mdc_adjustsFontForContentSizeCategory;
@property(readonly, nonatomic) OGLListItemControlStateStyling *normalStateStyling;
@property(readonly, nonatomic) UIView *selectedBackgroundView;
@property(readonly, nonatomic) UIImage *selectedIcon;
@property(readonly, nonatomic) OGLListItemControlStateStyling *selectedStateStyling;
@property(readonly, nonatomic) UIColor *selectedTextColor;
@property(readonly, nonatomic) _Bool shouldFlipIconForRTL;
@property(readonly, copy, nonatomic) NSString *subtitle;
@property(readonly, nonatomic) UIColor *subtitleTextColor;
@property(readonly) Class superclass;
@property(readonly, copy, nonatomic) NSString *title;
@property(readonly, nonatomic) unsigned long long titleTextSize;
@property(readonly, nonatomic) _Bool usedForTabNavigation;

@end

