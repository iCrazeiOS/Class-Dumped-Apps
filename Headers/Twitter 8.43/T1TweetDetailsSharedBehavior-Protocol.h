//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <T1Twitter/NSObject-Protocol.h>

@class T1AppNavigationContext, TFNTwitterAccount, TFSTwitterScribeContext;
@protocol T1StatusViewModel;

@protocol T1TweetDetailsSharedBehavior <NSObject>
@property(readonly, copy, nonatomic) TFSTwitterScribeContext *sourceScribeContext;
@property(retain, nonatomic) T1AppNavigationContext *navigationContext;
@property(readonly, nonatomic) id <T1StatusViewModel> initialFocusedStatus;
@property(readonly, nonatomic) id <T1StatusViewModel> focalStatus;
@property(readonly, nonatomic) TFNTwitterAccount *account;
@end

