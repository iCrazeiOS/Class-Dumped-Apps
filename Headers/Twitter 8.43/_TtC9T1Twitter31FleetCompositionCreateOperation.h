//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TFSUtilities/TFSDependentConcurrentOperation.h>

#import <T1Twitter/TFSOperationDependencyResult-Protocol.h>

@class NSError;

__attribute__((visibility("hidden")))
@interface _TtC9T1Twitter31FleetCompositionCreateOperation : TFSDependentConcurrentOperation <TFSOperationDependencyResult>
{
    // Error parsing type: , name: delegate
    // Error parsing type: , name: resultState
    // Error parsing type: , name: resultError
    // Error parsing type: , name: mediaUploadInfo
    // Error parsing type: , name: resultEntry
    // Error parsing type: , name: draft
    // Error parsing type: , name: context
    // Error parsing type: , name: operationQueue
    // Error parsing type: , name: backgroundTaskIdentifier
    // Error parsing type: , name: backgroundTaskSessionQueue
    // Error parsing type: , name: completionOperation
    // Error parsing type: , name: mediaUploadStartTime
    // Error parsing type: , name: mediaProcessingStartTime
    // Error parsing type: , name: mediaProcessingTime
    // Error parsing type: , name: mediaUploadError
}

- (void).cxx_destruct;
- (id)init;
- (void)completeOperation;
- (void)cancel;
- (void)run;
@property(nonatomic, copy) NSError *resultError;
@property(nonatomic) unsigned long long resultState; // @synthesize resultState;

@end

