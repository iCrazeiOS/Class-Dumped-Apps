//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface CMRBooleanSetting : NSObject
{
    _Bool _isSupported;
    _Bool _isInitialized;
    _Bool _value;
}

+ (id)BooleanSettingWithIsSupported:(_Bool)arg1 isInitialized:(_Bool)arg2 value:(_Bool)arg3;
@property(readonly, nonatomic) _Bool value; // @synthesize value=_value;
@property(readonly, nonatomic) _Bool isInitialized; // @synthesize isInitialized=_isInitialized;
@property(readonly, nonatomic) _Bool isSupported; // @synthesize isSupported=_isSupported;
- (id)description;
- (id)initWithIsSupported:(_Bool)arg1 isInitialized:(_Bool)arg2 value:(_Bool)arg3;

@end

