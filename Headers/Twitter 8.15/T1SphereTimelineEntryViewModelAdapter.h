//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TFNUI/TFNItemsDataViewAdapter.h>

@protocol T1SphereTimelineEntryViewModelAdapterDelegate;

@interface T1SphereTimelineEntryViewModelAdapter : TFNItemsDataViewAdapter
{
    // Error parsing type: , name: delegate
    // Error parsing type: , name: prototypeView
    // Error parsing type: , name: feedbackPresenter
    // Error parsing type: , name: $__lazy_storage_$_imagePipeline
    // Error parsing type: , name: account
    // Error parsing type: , name: listMutationsManager
    // Error parsing type: , name: $__lazy_storage_$_useCustomImageBanner
}

- (void).cxx_destruct;
- (id)init;
- (void)dataViewController:(id)arg1 didSelectItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4;
- (_Bool)dataViewController:(id)arg1 shouldSelectItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4;
- (id)dataViewController:(id)arg1 tableViewCellForItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4;
- (double)dataViewController:(id)arg1 tableViewHeightForItem:(id)arg2 withOptions:(id)arg3 atIndexPath:(id)arg4;
- (id)initWithAccount:(id)arg1;
@property(nonatomic) __weak id <T1SphereTimelineEntryViewModelAdapterDelegate> delegate; // @synthesize delegate;

@end

