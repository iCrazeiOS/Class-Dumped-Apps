//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <T1Twitter/T1CardViewBaseEventHandler.h>

#import <T1Twitter/T1CardViewEventHandler-Protocol.h>
#import <T1Twitter/TFNTwitterAudioPlayerDelegate-Protocol.h>

@class NSString, T1AudioCardView;

@interface T1AudioCardViewEventHandler : T1CardViewBaseEventHandler <TFNTwitterAudioPlayerDelegate, T1CardViewEventHandler>
{
    T1AudioCardView *_cardView;
}

- (void).cxx_destruct;
- (_Bool)audioPlayerCanPresent:(id)arg1;
- (void)audioPlayerDidStartLoading:(id)arg1;
- (void)audioPlayerDidPresent:(id)arg1;
- (void)audioPlayerDidDispose:(id)arg1;
- (void)_handleAudioPlayEvent:(id)arg1 context:(id)arg2;
- (id)handleViewEvent:(id)arg1 context:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

