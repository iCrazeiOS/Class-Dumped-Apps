//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface TFSLegacyModelSupport : NSObject
{
    // Error parsing type: , name: modelContext
}

- (void).cxx_destruct;
- (id)init;
- (void)statusesForStatusIDs:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (id)cardsForCardURLs:(id)arg1;
- (void)asyncUsersForUserIDs:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (id)usersForUserIDs:(id)arg1;
- (void)userForUsername:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (id)userForUserID:(long long)arg1;
- (id)initWithModelContext:(id)arg1;

@end

