//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSMutableDictionary, NSMutableSet, NSNotificationCenter;

@interface GDDNotifications : NSObject
{
    NSNotificationCenter *_notificationCenter;
    NSMutableDictionary *_blockObservers;
    NSMutableSet *_defaultCenterNames;
}

+ (id)defaultInstance;
- (void).cxx_destruct;
- (id)selectCenterForName:(id)arg1;
- (void)removeObserver:(id)arg1 name:(id)arg2;
- (void)removeObserver:(id)arg1;
- (void)addObserver:(id)arg1 name:(id)arg2 block:(CDUnknownBlockType)arg3 queue:(id)arg4;
- (void)addObserver:(id)arg1 name:(id)arg2 uiBlock:(CDUnknownBlockType)arg3;
- (void)addObserver:(id)arg1 name:(id)arg2 block:(CDUnknownBlockType)arg3;
- (void)addObserver:(id)arg1 selector:(SEL)arg2 name:(id)arg3;
- (void)postNotification:(id)arg1 object:(id)arg2 userInfo:(id)arg3;
- (void)postNotification:(id)arg1 userInfo:(id)arg2;
- (void)postNotification:(id)arg1 object:(id)arg2;
- (void)postNotification:(id)arg1;
- (id)init;

@end

