//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "ComGoogleCommonCollectStandardTable_RowMap.h"

#import "JavaUtilSortedMap-Protocol.h"

@class ComGoogleCommonCollectStandardRowSortedTable, NSString;

@interface ComGoogleCommonCollectStandardRowSortedTable_RowSortedMap : ComGoogleCommonCollectStandardTable_RowMap <JavaUtilSortedMap>
{
    ComGoogleCommonCollectStandardRowSortedTable *this$1_;
}

- (void)__javaClone:(id)arg1;
- (id)tailMapWithId:(id)arg1;
- (id)subMapWithId:(id)arg1 withId:(id)arg2;
- (id)headMapWithId:(id)arg1;
- (id)lastKey;
- (id)firstKey;
- (id)comparator;
- (id)createKeySet;
- (id)keySet;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

