//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <Module_Framework/YTOfflineEntityController-Protocol.h>
#import <Module_Framework/YTOfflineVideoControllerObserver-Protocol.h>

@class GIMMe, GPCPromise, NSString, YTIOfflineOrchestrationAction;

@interface YTOfflineSingleVideoTransferEntityController : NSObject <YTOfflineVideoControllerObserver, YTOfflineEntityController>
{
    GPCPromise *_executingActionPromise;
    YTIOfflineOrchestrationAction *_executingAction;
    NSString *_executingActionVideoID;
    GIMMe *_gimme;
}

+ (long long)entityType;
@property(nonatomic) __weak GIMMe *gimme; // @synthesize gimme=_gimme;
- (void).cxx_destruct;
- (void)resolveActionWithResult:(id)arg1;
- (void)successfullyResolveAction;
- (void)unsuccessfullyResolveActionWithRetryable:(_Bool)arg1;
- (void)checkExecutingActionMatchesVideoID:(id)arg1 andExecuteBlock:(CDUnknownBlockType)arg2;
- (void)fetchVideoFromStoreWithResponseBlock:(CDUnknownBlockType)arg1;
- (void)startDownloadForVideo:(id)arg1;
- (void)setVideoToPendingDownloadAndStartDownload;
- (void)verifyVideoExistsAndHasPlayerResponseWithCompletionBlock:(CDUnknownBlockType)arg1;
- (void)refreshEntity;
- (void)updateEntity;
- (void)deleteEntity;
- (void)addEntity;
- (void)executeAction;
- (void)didUpdateFromStatus:(int)arg1 toStatus:(int)arg2 forOfflineVideoWithID:(id)arg3;
- (void)downloadDidFailWithVideoID:(id)arg1 error:(id)arg2;
- (void)downloadDidCompleteWithVideoID:(id)arg1 pinTypes:(int)arg2;
- (id)handleAction:(id)arg1;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

