//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DYNSSyncInternalSyncer.h"

@class DYNSSyncV2RevisionedResponseHandler;
@protocol ComGoogleAppsXplatObserveSettable, DYNSAnalyticsClearcutEventsLogger, DYNSNetworkApiRequestManager, DYNSStorageApiTopicMessageStorageController, JavaxInjectProvider;

@interface DYNSSyncV2UpdateGroupRetentionSettingsSyncer : DYNSSyncInternalSyncer
{
    id <DYNSAnalyticsClearcutEventsLogger> clearcutEventsLogger_;
    id <JavaxInjectProvider> executorProvider_;
    id <ComGoogleAppsXplatObserveSettable> messageEventsSettable_;
    id <DYNSNetworkApiRequestManager> requestManager_;
    DYNSSyncV2RevisionedResponseHandler *revisionedResponseHandler_;
    id <DYNSStorageApiTopicMessageStorageController> topicMessageStorageController_;
}

+ (void)initialize;
- (void)dealloc;
- (id)executeWithDYNSSyncInternalSyncRequest:(id)arg1;

@end

