//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class DYNSDataUiGroupSummariesConverter, XPTXTracer;
@protocol DYNSAnalyticsClearcutStreamzLogger, DYNSStorageApiWorldStorageCoordinator, JavaxInjectProvider;

@interface DYNSActionsGetLocalGroupSummariesAction : NSObject
{
    XPTXTracer *tracer_;
    id <DYNSAnalyticsClearcutStreamzLogger> clearcutStreamzLogger_;
    id <JavaxInjectProvider> executorProvider_;
    DYNSDataUiGroupSummariesConverter *uiGroupSummariesConverter_;
    id <DYNSStorageApiWorldStorageCoordinator> worldStorageCoordinator_;
}

- (void)dealloc;
- (id)execute;

@end

