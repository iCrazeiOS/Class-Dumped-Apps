//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "GAVRenderer-Protocol.h"
#import "GAVWebViewDelegate-Protocol.h"

@class GAVProjectorConfiguration, GAVRendererTracker, GAVWebView, NSURL;
@protocol GAVFileInfo, GAVRendererDelegate;

@interface GAVWebViewerVC : UIViewController <GAVWebViewDelegate, GAVRenderer>
{
    _Bool _chromeHidden;
    _Bool _isRotating;
    _Bool _disableLongPressGesture;
    id <GAVRendererDelegate> _delegate;
    GAVProjectorConfiguration *_projectorConfiguration;
    id <GAVFileInfo> _fileInfo;
    GAVWebView *_webView;
    NSURL *_temporaryFileURL;
    GAVRendererTracker *_rendererTracker;
}

+ (id)fileExtensionForMIMEType:(id)arg1;
- (void).cxx_destruct;
@property(retain, nonatomic) GAVRendererTracker *rendererTracker; // @synthesize rendererTracker=_rendererTracker;
@property(retain, nonatomic) NSURL *temporaryFileURL; // @synthesize temporaryFileURL=_temporaryFileURL;
@property(retain, nonatomic) GAVWebView *webView; // @synthesize webView=_webView;
@property(readonly, nonatomic) _Bool disableLongPressGesture; // @synthesize disableLongPressGesture=_disableLongPressGesture;
@property(nonatomic) _Bool isRotating; // @synthesize isRotating=_isRotating;
@property(nonatomic) _Bool chromeHidden; // @synthesize chromeHidden=_chromeHidden;
@property(copy, nonatomic) id <GAVFileInfo> fileInfo; // @synthesize fileInfo=_fileInfo;
@property(retain, nonatomic) GAVProjectorConfiguration *projectorConfiguration; // @synthesize projectorConfiguration=_projectorConfiguration;
@property(nonatomic) __weak id <GAVRendererDelegate> delegate; // @synthesize delegate=_delegate;
- (void)failWithNoPreviewAvailableError;
- (void)webViewDidReceiveTap:(id)arg1;
- (void)webView:(id)arg1 didChangeVerticalContentOffset:(double)arg2;
- (void)webView:(id)arg1 didZoomToScale:(double)arg2 withVerticalContentOffset:(double)arg3;
- (void)webView:(id)arg1 requestedToOpenURL:(id)arg2;
- (void)rendererDidBecomeVisible;
- (_Bool)restoreToOriginalHeight:(double)arg1;
- (void)didShowBottomViewControllerWithHeight:(double)arg1 rendererHeight:(double)arg2;
- (void)viewWillTransitionToSize:(struct CGSize)arg1 withTransitionCoordinator:(id)arg2;
- (void)loadView;
- (void)dealloc;
- (id)initWithFileInfo:(id)arg1 disableLongPressGesture:(_Bool)arg2;

@end

