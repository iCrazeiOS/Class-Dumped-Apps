//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "ComGoogleCommonUtilConcurrentRateLimiter.h"

@class JavaLangInteger;
@protocol JavaUtilList;

@interface ComGoogleCommonUtilConcurrentSecondGranularityHorizonRateLimiter : ComGoogleCommonUtilConcurrentRateLimiter
{
    int permitsWithinHorizon_;
    int horizonSec_;
    id <JavaUtilList> permitGrantsSec_;
    JavaLangInteger *latestPermitGrantSec_;
}

- (void)dealloc;
- (void)doSetPermitsWithinHorizonWithInt:(int)arg1;
- (void)doForceAcquireWithLong:(long long)arg1;
- (int)doLatestPermitAgeSecWithLong:(long long)arg1;
- (long long)reserveEarliestAvailableWithInt:(int)arg1 withLong:(long long)arg2;
- (long long)queryEarliestAvailableWithLong:(long long)arg1;
- (double)doGetRate;
- (void)doSetRateWithDouble:(double)arg1 withLong:(long long)arg2;

@end

