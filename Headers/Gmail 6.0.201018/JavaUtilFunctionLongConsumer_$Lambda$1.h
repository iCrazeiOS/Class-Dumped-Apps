//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "JavaUtilFunctionLongConsumer-Protocol.h"

@class NSString;
@protocol JavaUtilFunctionLongConsumer;

@interface JavaUtilFunctionLongConsumer_$Lambda$1 : NSObject <JavaUtilFunctionLongConsumer>
{
    id <JavaUtilFunctionLongConsumer> this$0_;
    id <JavaUtilFunctionLongConsumer> val$after_;
}

- (void)dealloc;
- (id)andThenWithJavaUtilFunctionLongConsumer:(id)arg1;
- (void)acceptWithLong:(long long)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

