//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DYNSStorageProcessorsAbstractEventsProcessor.h"

@class DYNSDataModelsConvertersUserStatusConverter, DYNSDataUiUserStatusConverter, DYNSSyncV2SubscriptionsEventDispatcher;
@protocol ComGoogleAppsDynamiteV1SharedUtilAccountuserAccountUser, DYNSCommonDynamiteClock, DYNSStatusApiAccountOwnerStatusManager, DYNSStatusApiUserStatusManager, DYNSStorageControllersUserStatusStorageControllerInternal;

@interface DYNSStorageProcessorsUserStatusEventsProcessor : DYNSStorageProcessorsAbstractEventsProcessor
{
    id <DYNSStatusApiAccountOwnerStatusManager> accountOwnerStatusManager_;
    id <ComGoogleAppsDynamiteV1SharedUtilAccountuserAccountUser> accountUser_;
    DYNSDataModelsConvertersUserStatusConverter *userStatusConverter_;
    id <DYNSStorageControllersUserStatusStorageControllerInternal> userStatusStorageControllerInternal_;
    id <DYNSCommonDynamiteClock> dynamiteClock_;
    DYNSSyncV2SubscriptionsEventDispatcher *eventDispatcher_;
    DYNSDataUiUserStatusConverter *uiUserStatusConverter_;
    id <DYNSStatusApiUserStatusManager> userStatusManager_;
}

+ (void)initialize;
- (void)dealloc;
- (void)postProcessEventResultsWithBoolean:(_Bool)arg1 withId:(id)arg2;
- (id)writeToDatabaseWithComGoogleAppsXplatSqlSqlTransaction:(id)arg1 withId:(id)arg2;
- (id)processEventBodyWithId:(id)arg1 withId:(id)arg2;
- (id)readFromDatabaseWithComGoogleAppsXplatSqlSqlTransaction:(id)arg1 withId:(id)arg2;
- (id)preProcessEventBodiesWithComGoogleCommonCollectImmutableList:(id)arg1;

@end

