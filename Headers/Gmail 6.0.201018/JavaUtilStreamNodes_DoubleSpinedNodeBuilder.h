//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "JavaUtilStreamSpinedBuffer_OfDouble.h"

#import "JavaUtilStreamNode_Builder_OfDouble-Protocol.h"
#import "JavaUtilStreamNode_OfDouble-Protocol.h"

@class NSString;

@interface JavaUtilStreamNodes_DoubleSpinedNodeBuilder : JavaUtilStreamSpinedBuffer_OfDouble <JavaUtilStreamNode_OfDouble, JavaUtilStreamNode_Builder_OfDouble>
{
    _Bool building_;
}

+ (const struct J2ObjcClassInfo *)__metadata;
- (unsigned long long)countByEnumeratingWithState:(CDStruct_58648341 *)arg1 objects:(id *)arg2 count:(unsigned long long)arg3;
- (id)andThenWithJavaUtilFunctionConsumer:(id)arg1;
- (void)acceptWithId:(id)arg1;
- (void)acceptWithLong:(long long)arg1;
- (void)acceptWithInt:(int)arg1;
- (_Bool)cancellationRequested;
- (int)getChildCount;
- (id)asArrayWithJavaUtilFunctionIntFunction:(id)arg1;
- (id)getChildWithInt:(int)arg1;
- (id)getShape;
- (id)truncateWithLong:(long long)arg1 withLong:(long long)arg2 withJavaUtilFunctionIntFunction:(id)arg3;
- (void)copyIntoWithNSObjectArray:(id)arg1 withInt:(int)arg2;
- (id)build;
- (id)asPrimitiveArray;
- (void)copyIntoWithId:(id)arg1 withInt:(int)arg2;
- (void)end;
- (void)acceptWithDouble:(double)arg1;
- (void)beginWithLong:(long long)arg1;
- (void)forEachWithId:(id)arg1;
- (id)spliterator;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

