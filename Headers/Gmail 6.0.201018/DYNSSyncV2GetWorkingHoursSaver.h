//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DYNSSyncInternalSyncer.h"

@class DYNSSyncV2EntitiesEntityManagerUtils;
@protocol DYNSStorageApiWorkingHoursStorageController, JavaxInjectProvider;

@interface DYNSSyncV2GetWorkingHoursSaver : DYNSSyncInternalSyncer
{
    DYNSSyncV2EntitiesEntityManagerUtils *entityManagerUtils_;
    id <JavaxInjectProvider> executorProvider_;
    id <DYNSStorageApiWorkingHoursStorageController> workingHoursStorageController_;
}

- (void)dealloc;
- (id)executeWithDYNSSyncInternalSyncRequest:(id)arg1;

@end

