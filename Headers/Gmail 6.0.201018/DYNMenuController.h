//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "GOOPopoverViewControllerDelegate-Protocol.h"

@class GOOPopoverViewController, NSArray, NSString, UIViewController;

@interface DYNMenuController : NSObject <GOOPopoverViewControllerDelegate>
{
    GOOPopoverViewController *_popoverViewController;
    NSArray *_menuItems;
    UIViewController *_presenter;
    NSString *_menuTitle;
}

- (void).cxx_destruct;
@property(copy, nonatomic) NSString *menuTitle; // @synthesize menuTitle=_menuTitle;
@property(nonatomic) __weak UIViewController *presenter; // @synthesize presenter=_presenter;
@property(retain, nonatomic) NSArray *menuItems; // @synthesize menuItems=_menuItems;
- (void)popoverViewControllerDidCancel:(id)arg1;
- (void)dismissAnimated:(_Bool)arg1 completion:(CDUnknownBlockType)arg2;
- (void)showMenuWithSourceView:(id)arg1 menuAnchor:(unsigned long long)arg2;
- (void)showMenuWithSourceView:(id)arg1;
- (id)initWithPresenter:(id)arg1 menuItems:(id)arg2 title:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

