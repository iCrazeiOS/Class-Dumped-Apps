//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DYNSSyncInternalSyncer.h"

@class DYNSDataModelsConvertersMessageConverter, DYNSDataUiMessageConverter, DYNSSyncV2RevisionedResponseHandler;
@protocol DYNSNetworkApiRequestManager, JavaxInjectProvider;

@interface DYNSSyncV2EditMessageSyncer : DYNSSyncInternalSyncer
{
    id <JavaxInjectProvider> executorProvider_;
    DYNSDataModelsConvertersMessageConverter *messageConverter_;
    id <DYNSNetworkApiRequestManager> requestManager_;
    DYNSSyncV2RevisionedResponseHandler *revisionedResponseHandler_;
    DYNSDataUiMessageConverter *uiMessageConverter_;
}

- (void)dealloc;
- (id)executeWithDYNSSyncInternalSyncRequest:(id)arg1;

@end

