//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DYNSSyncCommonGetMembersSyncer_ParsedGetMembersResponse.h"

@class ComGoogleCommonCollectImmutableList, ComGoogleCommonCollectImmutableSet;

@interface DYNSSyncCommonAutoValue_GetMembersSyncer_ParsedGetMembersResponse : DYNSSyncCommonGetMembersSyncer_ParsedGetMembersResponse
{
    ComGoogleCommonCollectImmutableList *users_;
    ComGoogleCommonCollectImmutableList *rosters_;
    ComGoogleCommonCollectImmutableSet *omittedMemberIds_;
    ComGoogleCommonCollectImmutableSet *anonymousUserContextIds_;
}

- (void)dealloc;
- (unsigned long long)hash;
- (_Bool)isEqual:(id)arg1;
- (id)description;
- (id)getAnonymousUserContextIds;
- (id)getOmittedMemberIds;
- (id)getRosters;
- (id)getUsers;

@end

