//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "HTSLiveMessageSubscriber-Protocol.h"
#import "IESLiveMTFeedMessageListActions-Protocol.h"

@class IESLiveMTFeedMessageListNodeFactory, NSArray, NSMutableArray, NSNumber, NSString, RACDisposable;
@protocol HTSLiveMessageDispatchProvider, IESLiveMTFeedMessageListReactions, IESLiveSettings, OS_dispatch_queue, OS_dispatch_source;

@interface IESLiveMTFeedMessageListStore : NSObject <HTSLiveMessageSubscriber, IESLiveMTFeedMessageListActions>
{
    _Bool _reachLowMemmoryThreshold;
    _Bool _hasNeedDeleteMessageNode;
    _Bool _hasNewCommonMessageNode;
    _Bool _disableAnimation;
    _Bool _updating;
    _Bool _isMessageListCrashProtectionEnabled;
    _Bool _messageSwitch;
    id <IESLiveMTFeedMessageListReactions> _reactions;
    NSArray *_messages;
    NSArray *_exceededIndexPathes;
    NSArray *_addedIndexPathes;
    NSNumber *_roomID;
    RACDisposable *_disposable;
    id <HTSLiveMessageDispatchProvider> _messageDispatcher;
    IESLiveMTFeedMessageListNodeFactory *_messageNodeFactory;
    NSObject<OS_dispatch_queue> *_messageProcessQueue;
    NSMutableArray *_buffer;
    NSObject<OS_dispatch_source> *_timer;
    long long _refreshCount;
    id <IESLiveSettings> _settings;
    double _lastRefreshDataSourceTimestamp;
    double _refreshDataSourceMinumInterval;
    long long _pileUpChatMessageNum;
    long long _pileUpEnterMessageNum;
    NSObject<OS_dispatch_source> *_loopMessageTimer;
}

- (void).cxx_destruct;
@property(retain, nonatomic) NSObject<OS_dispatch_source> *loopMessageTimer; // @synthesize loopMessageTimer=_loopMessageTimer;
@property(nonatomic) long long pileUpEnterMessageNum; // @synthesize pileUpEnterMessageNum=_pileUpEnterMessageNum;
@property(nonatomic) long long pileUpChatMessageNum; // @synthesize pileUpChatMessageNum=_pileUpChatMessageNum;
@property(nonatomic) _Bool messageSwitch; // @synthesize messageSwitch=_messageSwitch;
@property(nonatomic) double refreshDataSourceMinumInterval; // @synthesize refreshDataSourceMinumInterval=_refreshDataSourceMinumInterval;
@property(nonatomic) double lastRefreshDataSourceTimestamp; // @synthesize lastRefreshDataSourceTimestamp=_lastRefreshDataSourceTimestamp;
@property(nonatomic) _Bool isMessageListCrashProtectionEnabled; // @synthesize isMessageListCrashProtectionEnabled=_isMessageListCrashProtectionEnabled;
@property(retain, nonatomic) id <IESLiveSettings> settings; // @synthesize settings=_settings;
@property(nonatomic) _Bool updating; // @synthesize updating=_updating;
@property(nonatomic) long long refreshCount; // @synthesize refreshCount=_refreshCount;
@property(retain, nonatomic) NSObject<OS_dispatch_source> *timer; // @synthesize timer=_timer;
@property(retain, nonatomic) NSMutableArray *buffer; // @synthesize buffer=_buffer;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *messageProcessQueue; // @synthesize messageProcessQueue=_messageProcessQueue;
@property(retain, nonatomic) IESLiveMTFeedMessageListNodeFactory *messageNodeFactory; // @synthesize messageNodeFactory=_messageNodeFactory;
@property(retain, nonatomic) id <HTSLiveMessageDispatchProvider> messageDispatcher; // @synthesize messageDispatcher=_messageDispatcher;
@property(retain, nonatomic) RACDisposable *disposable; // @synthesize disposable=_disposable;
@property(copy, nonatomic) NSNumber *roomID; // @synthesize roomID=_roomID;
@property(nonatomic) _Bool disableAnimation; // @synthesize disableAnimation=_disableAnimation;
@property(nonatomic) _Bool hasNewCommonMessageNode; // @synthesize hasNewCommonMessageNode=_hasNewCommonMessageNode;
@property(retain, nonatomic) NSArray *addedIndexPathes; // @synthesize addedIndexPathes=_addedIndexPathes;
@property(nonatomic) _Bool hasNeedDeleteMessageNode; // @synthesize hasNeedDeleteMessageNode=_hasNeedDeleteMessageNode;
@property(retain, nonatomic) NSArray *exceededIndexPathes; // @synthesize exceededIndexPathes=_exceededIndexPathes;
@property(retain, nonatomic) NSArray *messages; // @synthesize messages=_messages;
@property(nonatomic) _Bool reachLowMemmoryThreshold; // @synthesize reachLowMemmoryThreshold=_reachLowMemmoryThreshold;
@property(nonatomic) __weak id <IESLiveMTFeedMessageListReactions> reactions; // @synthesize reactions=_reactions;
- (double)messageDelayTimeSettingWithKey:(id)arg1;
- (void)startLoopMessageTimerAfterDelay:(double)arg1;
- (void)reloadMessages:(CDUnknownBlockType)arg1;
- (void)disableAnimationIfNeeded;
@property(readonly, nonatomic) double currentListWidth;
- (void)checkMemoryUsageIfNeeded;
- (void)setupTimer;
- (void)refreshMessageDataSource;
- (void)update;
- (void)successCreatedNode:(id)arg1;
- (void)liveFinished;
- (void)disableUpdate;
- (void)enableUpdate;
- (void)setupTopGradient;
- (void)resetMessageListWidth;
- (void)updateMessageListWidth:(double)arg1;
- (void)messageReceived:(id)arg1;
- (void)createCombineMessage;
- (void)startShowMessages:(id)arg1 shouldShowKeyMessage:(_Bool)arg2 shouldShowRollingMessage:(_Bool)arg3;
- (void)setup;
- (id)initWithRoomID:(id)arg1;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

