//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface TIMPBNParticipant : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(copy, nonatomic) NSString *alias; // @dynamic alias;
@property(nonatomic) int blocked; // @dynamic blocked;
@property(nonatomic) _Bool hasAlias; // @dynamic hasAlias;
@property(nonatomic) _Bool hasBlocked; // @dynamic hasBlocked;
@property(nonatomic) _Bool hasLeftBlockTime; // @dynamic hasLeftBlockTime;
@property(nonatomic) _Bool hasRole; // @dynamic hasRole;
@property(nonatomic) _Bool hasSecUid; // @dynamic hasSecUid;
@property(nonatomic) _Bool hasSortOrder; // @dynamic hasSortOrder;
@property(nonatomic) _Bool hasUserId; // @dynamic hasUserId;
@property(nonatomic) long long leftBlockTime; // @dynamic leftBlockTime;
@property(nonatomic) int role; // @dynamic role;
@property(copy, nonatomic) NSString *secUid; // @dynamic secUid;
@property(nonatomic) long long sortOrder; // @dynamic sortOrder;
@property(nonatomic) long long userId; // @dynamic userId;

@end

