//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSDate, NSDictionary, NSNumber, NSString;

@interface HMDInjectedInfo : NSObject
{
    struct _opaque_pthread_rwlock_t commonParamsRWLock;
    struct _opaque_pthread_rwlock_t didRWLock;
    struct _opaque_pthread_rwlock_t iidRWLock;
    struct _opaque_pthread_rwlock_t uidRWLock;
    struct _opaque_pthread_rwlock_t ignoreTimeRWLock;
    _Bool _ignorePIPESignalCrash;
    NSDictionary *_commonParams;
    NSString *_allUploadHost;
    NSString *_deviceID;
    NSString *_installID;
    NSString *_userID;
    NSDate *_ignorePerformanceDataTime;
    NSString *_appID;
    NSString *_appName;
    NSString *_channel;
    NSString *_userName;
    NSString *_email;
    NSString *_sessionID;
    CDUnknownBlockType _transformBlock;
    CDUnknownBlockType _commonParamsBlock;
    unsigned long long _monitorFlushCount;
    NSString *_appGroupID;
    NSDictionary *_customContext;
    CDUnknownBlockType _networkParamsBlock;
    NSDictionary *_customHeader;
    NSString *_crashUploadHost;
    NSString *_exceptionUploadHost;
    NSString *_userExceptionUploadHost;
    NSString *_performanceUploadHost;
    NSString *_fileUploadHost;
    NSArray *_configHostArray;
    NSString *_business;
    NSDictionary *_filters;
    NSArray *_defaultSetupModules;
    CDUnknownBlockType _dynamicDID;
    CDUnknownBlockType _dynamicIID;
    CDUnknownBlockType _dynamicUID;
    NSString *_buildInfo;
}

+ (id)allocWithZone:(struct _NSZone *)arg1;
+ (id)defaultInfo;
@property(copy, nonatomic) NSString *buildInfo; // @synthesize buildInfo=_buildInfo;
@property(copy) CDUnknownBlockType dynamicUID; // @synthesize dynamicUID=_dynamicUID;
@property(copy) CDUnknownBlockType dynamicIID; // @synthesize dynamicIID=_dynamicIID;
@property(copy) CDUnknownBlockType dynamicDID; // @synthesize dynamicDID=_dynamicDID;
@property(copy, nonatomic) NSArray *defaultSetupModules; // @synthesize defaultSetupModules=_defaultSetupModules;
@property(copy) NSDictionary *filters; // @synthesize filters=_filters;
@property(copy) NSString *business; // @synthesize business=_business;
@property(copy, nonatomic) NSArray *configHostArray; // @synthesize configHostArray=_configHostArray;
@property(copy, nonatomic) NSString *fileUploadHost; // @synthesize fileUploadHost=_fileUploadHost;
@property(copy, nonatomic) NSString *performanceUploadHost; // @synthesize performanceUploadHost=_performanceUploadHost;
@property(copy, nonatomic) NSString *userExceptionUploadHost; // @synthesize userExceptionUploadHost=_userExceptionUploadHost;
@property(copy, nonatomic) NSString *exceptionUploadHost; // @synthesize exceptionUploadHost=_exceptionUploadHost;
@property(copy, nonatomic) NSString *crashUploadHost; // @synthesize crashUploadHost=_crashUploadHost;
@property(copy) NSDictionary *customHeader; // @synthesize customHeader=_customHeader;
@property(copy) CDUnknownBlockType networkParamsBlock; // @synthesize networkParamsBlock=_networkParamsBlock;
@property(copy) NSDictionary *customContext; // @synthesize customContext=_customContext;
@property(copy, nonatomic) NSString *appGroupID; // @synthesize appGroupID=_appGroupID;
@property(nonatomic) unsigned long long monitorFlushCount; // @synthesize monitorFlushCount=_monitorFlushCount;
@property(nonatomic) _Bool ignorePIPESignalCrash; // @synthesize ignorePIPESignalCrash=_ignorePIPESignalCrash;
@property(copy) CDUnknownBlockType commonParamsBlock; // @synthesize commonParamsBlock=_commonParamsBlock;
@property(copy) CDUnknownBlockType transformBlock; // @synthesize transformBlock=_transformBlock;
@property(copy) NSString *sessionID; // @synthesize sessionID=_sessionID;
@property(copy) NSString *email; // @synthesize email=_email;
@property(copy) NSString *userName; // @synthesize userName=_userName;
@property(copy, nonatomic) NSString *channel; // @synthesize channel=_channel;
@property(copy, nonatomic) NSString *appName; // @synthesize appName=_appName;
@property(copy, nonatomic) NSString *appID; // @synthesize appID=_appID;
@property(copy, nonatomic) NSString *allUploadHost; // @synthesize allUploadHost=_allUploadHost;
- (void).cxx_destruct;
- (double)getIgnorePerformanceDataTimeInterval;
@property(retain, nonatomic) NSDate *ignorePerformanceDataTime; // @synthesize ignorePerformanceDataTime=_ignorePerformanceDataTime;
@property(copy) NSString *userID; // @synthesize userID=_userID;
@property(copy) NSString *installID; // @synthesize installID=_installID;
@property(copy) NSString *deviceID; // @synthesize deviceID=_deviceID;
- (void)removeCustomHeaderKey:(id)arg1;
- (void)setCustomHeaderValue:(id)arg1 forKey:(id)arg2;
- (void)addBytestFilterIfNeed;
- (void)removeCustomFilterKey:(id)arg1;
- (void)setCustomFilterValue:(id)arg1 forKey:(id)arg2;
- (void)removeCustomContextKey:(id)arg1;
- (void)setCustomContextValue:(id)arg1 forKey:(id)arg2;
- (id)ssAppID;
- (id)getCurrentChannel;
- (id)defaultAppName;
@property(copy) NSDictionary *commonParams; // @synthesize commonParams=_commonParams;
- (id)getTTNetParamsIfAvailable;
- (id)init;
- (id)alogUploadCommonParams;
- (_Bool)enableBackgroundUpload;
- (id)reportPerformanceURLPath;
- (id)transformedURLStringFrom:(id)arg1;
- (id)reportCommonParams;
- (id)reportHeaderParams;
@property(retain, nonatomic) NSNumber *disableNetworkRequest;
- (void)confUniqueKeyForData:(id)arg1 timestamp:(long long)arg2 eventType:(id)arg3;

@end

