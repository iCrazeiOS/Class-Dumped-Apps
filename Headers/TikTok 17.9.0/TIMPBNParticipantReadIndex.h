//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface TIMPBNParticipantReadIndex : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) _Bool hasIndex; // @dynamic hasIndex;
@property(nonatomic) _Bool hasIndexV2; // @dynamic hasIndexV2;
@property(nonatomic) _Bool hasSecUid; // @dynamic hasSecUid;
@property(nonatomic) _Bool hasUserId; // @dynamic hasUserId;
@property(nonatomic) long long index; // @dynamic index;
@property(nonatomic) long long indexV2; // @dynamic indexV2;
@property(copy, nonatomic) NSString *secUid; // @dynamic secUid;
@property(nonatomic) long long userId; // @dynamic userId;

@end

