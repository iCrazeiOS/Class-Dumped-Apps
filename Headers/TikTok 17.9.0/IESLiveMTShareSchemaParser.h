//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "IESLiveMTSchemaParser-Protocol.h"

@class NSString;
@protocol HTSLiveDetailRouter, HTSLiveViewHierarchyProvider, IESLiveMTCommentRouter, IESLiveRoomService, IESLiveShareService;

@interface IESLiveMTShareSchemaParser : NSObject <IESLiveMTSchemaParser>
{
    id <IESLiveShareService> _shareConfig;
    id <HTSLiveDetailRouter> _router;
    id <IESLiveRoomService> _roomService;
    id <HTSLiveViewHierarchyProvider> _viewProvider;
    id <IESLiveMTCommentRouter> _commentRouter;
}

+ (void)ieslivekit_register_schema;
- (void).cxx_destruct;
@property(retain, nonatomic) id <IESLiveMTCommentRouter> commentRouter; // @synthesize commentRouter=_commentRouter;
@property(retain, nonatomic) id <HTSLiveViewHierarchyProvider> viewProvider; // @synthesize viewProvider=_viewProvider;
@property(retain, nonatomic) id <IESLiveRoomService> roomService; // @synthesize roomService=_roomService;
@property(retain, nonatomic) id <HTSLiveDetailRouter> router; // @synthesize router=_router;
@property(retain, nonatomic) id <IESLiveShareService> shareConfig; // @synthesize shareConfig=_shareConfig;
- (void)hideCommentView;
- (void)parseWithParams:(id)arg1 fromInside:(_Bool)arg2 completion:(CDUnknownBlockType)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

