//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "AWEListDataController.h"

@class AWEFavoriteCollectionModel, NSNumber;

@interface AWEFavoriteMovieDataController : AWEListDataController
{
    NSNumber *_cursor;
    AWEFavoriteCollectionModel *_justRemovedModel;
}

- (void).cxx_destruct;
@property(retain, nonatomic) AWEFavoriteCollectionModel *justRemovedModel; // @synthesize justRemovedModel=_justRemovedModel;
@property(retain, nonatomic) NSNumber *cursor; // @synthesize cursor=_cursor;
- (_Bool)addAwemeWithItemID:(id)arg1;
- (_Bool)removeWithItemID:(id)arg1;
- (void)loadMoreWithCompletion:(CDUnknownBlockType)arg1;
- (void)refreshWithCompletion:(CDUnknownBlockType)arg1;
- (void)initFetchWithCompletion:(CDUnknownBlockType)arg1;
- (void)fetchDataWithCompletion:(CDUnknownBlockType)arg1 isLoadmore:(_Bool)arg2;
- (id)init;

@end

