//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SKStoreProductViewControllerDelegate-Protocol.h"

@class BDAAppStorePreloadItem, BDAAppStorePreloadList, NSString;

@interface BDAAppStoreManager : NSObject <SKStoreProductViewControllerDelegate>
{
    _Bool _preloadOnlyWIFI;
    _Bool _enablePreventSKGesture;
    _Bool _isReusePreload;
    _Bool _isPresentAnimated;
    _Bool _presenting;
    BDAAppStorePreloadList *_preloadList;
    BDAAppStorePreloadItem *_presentingItem;
}

+ (_Bool)isAppStoreURL:(id)arg1;
+ (id)appIDInURL:(id)arg1;
+ (id)duplicatedControllerWithAppID:(id)arg1 dismiassAnimated:(_Bool)arg2;
+ (void)clearAllPreloadCache;
+ (void)clearPreloadCacheBySKController:(id)arg1;
+ (void)clearPreloadCacheByAppID:(id)arg1;
+ (_Bool)hasControllerPreloadedByAppID:(id)arg1;
+ (id)controllerPreloadedByAppID:(id)arg1;
+ (void)openAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 downloadUrl:(id)arg3 adID:(id)arg4 logExtra:(id)arg5 byController:(id)arg6 presentCompletion:(CDUnknownBlockType)arg7 dismissCompletion:(CDUnknownBlockType)arg8;
+ (void)openAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 adID:(id)arg3 logExtra:(id)arg4 byController:(id)arg5 presentCompletion:(CDUnknownBlockType)arg6 dismissCompletion:(CDUnknownBlockType)arg7;
+ (void)openAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 byController:(id)arg3 presentCompletion:(CDUnknownBlockType)arg4 dismissCompletion:(CDUnknownBlockType)arg5;
+ (void)openAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 byController:(id)arg3;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 adID:(id)arg3 logExtra:(id)arg4 isDuplicate:(_Bool)arg5 completion:(CDUnknownBlockType)arg6;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 adID:(id)arg3 logExtra:(id)arg4 completion:(CDUnknownBlockType)arg5;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 completion:(CDUnknownBlockType)arg3;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2;
+ (void)openAppStoreControllerWithAppID:(id)arg1 downloadUrl:(id)arg2 adID:(id)arg3 logExtra:(id)arg4 byController:(id)arg5 presentCompletion:(CDUnknownBlockType)arg6 dismissCompletion:(CDUnknownBlockType)arg7;
+ (void)openAppStoreControllerWithAppID:(id)arg1 adID:(id)arg2 logExtra:(id)arg3 byController:(id)arg4 presentCompletion:(CDUnknownBlockType)arg5 dismissCompletion:(CDUnknownBlockType)arg6;
+ (void)openAppStoreControllerWithAppID:(id)arg1 byController:(id)arg2 presentCompletion:(CDUnknownBlockType)arg3 dismissCompletion:(CDUnknownBlockType)arg4;
+ (void)openAppStoreControllerWithAppID:(id)arg1 byController:(id)arg2;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 adID:(id)arg2 logExtra:(id)arg3 isDuplicate:(_Bool)arg4 completion:(CDUnknownBlockType)arg5;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 adID:(id)arg2 logExtra:(id)arg3 completion:(CDUnknownBlockType)arg4;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1 completion:(CDUnknownBlockType)arg2;
+ (void)preloadAppStoreControllerWithAppID:(id)arg1;
+ (id)sharedManager;
@property(retain, nonatomic) BDAAppStorePreloadItem *presentingItem; // @synthesize presentingItem=_presentingItem;
@property(retain, nonatomic) BDAAppStorePreloadList *preloadList; // @synthesize preloadList=_preloadList;
@property(nonatomic) _Bool presenting; // @synthesize presenting=_presenting;
@property(nonatomic) _Bool isPresentAnimated; // @synthesize isPresentAnimated=_isPresentAnimated;
@property(nonatomic) _Bool isReusePreload; // @synthesize isReusePreload=_isReusePreload;
@property(nonatomic) _Bool enablePreventSKGesture; // @synthesize enablePreventSKGesture=_enablePreventSKGesture;
@property(nonatomic) _Bool preloadOnlyWIFI; // @synthesize preloadOnlyWIFI=_preloadOnlyWIFI;
- (void).cxx_destruct;
- (void)clearAllPreloadCache;
- (void)clearPreloadCacheBySKController:(id)arg1;
- (void)clearPreloadCacheByAppID:(id)arg1;
- (void)openAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 downloadUrl:(id)arg3 adID:(id)arg4 logExtra:(id)arg5 byController:(id)arg6 presentCompletion:(CDUnknownBlockType)arg7 dismissCompletion:(CDUnknownBlockType)arg8;
- (_Bool)hasControllerPreloadedByAppID:(id)arg1;
- (id)controllerPreloadedByAppID:(id)arg1;
- (void)preloadAppStoreControllerWithAppID:(id)arg1 extraParams:(id)arg2 adID:(id)arg3 logExtra:(id)arg4 isDuplicate:(_Bool)arg5 completion:(CDUnknownBlockType)arg6;
- (id)duplicatedControllerPreloadedByAppID:(id)arg1 dismiassAnimated:(_Bool)arg2;
- (_Bool)shouldStartPreloadWithAppID:(id)arg1 error:(id *)arg2;
@property(readonly, nonatomic) long long supportPreloadType;
@property(nonatomic) unsigned long long maxPreloadCount;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

