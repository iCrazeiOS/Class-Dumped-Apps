//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "IESAssetReverseProtocol-Protocol.h"

@class AVURLAsset, IESMMMediaExporter, NSString, NSURL;

@interface IESAVAssetReverse : NSObject <IESAssetReverseProtocol>
{
    _Bool _shouldCancelReverse;
    AVURLAsset *_asset;
    IESMMMediaExporter *_audioExporter;
    NSURL *_reverseAudioUrl;
}

@property(retain, nonatomic) NSURL *reverseAudioUrl; // @synthesize reverseAudioUrl=_reverseAudioUrl;
@property(retain, nonatomic) IESMMMediaExporter *audioExporter; // @synthesize audioExporter=_audioExporter;
@property(retain, nonatomic) AVURLAsset *asset; // @synthesize asset=_asset;
@property(nonatomic) _Bool shouldCancelReverse; // @synthesize shouldCancelReverse=_shouldCancelReverse;
- (void).cxx_destruct;
- (void)dealloc;
- (id)videoCompsitionWithAsset:(id)arg1;
- (void)cancle;
- (id)p_getCompressionProps;
- (void)reverseAsset:(CDUnknownBlockType)arg1 progressBlock:(CDUnknownBlockType)arg2;
- (void)appWillResignAction:(id)arg1;
- (id)initWithAVAsset:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

