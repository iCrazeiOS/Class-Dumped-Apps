//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "AWEBaseApiModel.h"

@class AWEURLModel, NSNumber, NSString;

@interface AWEVideoTagModel : AWEBaseApiModel
{
    NSString *_title;
    NSString *_textColor;
    NSString *_backgroundColor;
    AWEURLModel *_iconImageURL;
    NSNumber *_leftRightPadding;
}

+ (id)iconImageURLJSONTransformer;
+ (id)JSONKeyPathsByPropertyKey;
- (void).cxx_destruct;
@property(retain, nonatomic) NSNumber *leftRightPadding; // @synthesize leftRightPadding=_leftRightPadding;
@property(copy, nonatomic) AWEURLModel *iconImageURL; // @synthesize iconImageURL=_iconImageURL;
@property(copy, nonatomic) NSString *backgroundColor; // @synthesize backgroundColor=_backgroundColor;
@property(copy, nonatomic) NSString *textColor; // @synthesize textColor=_textColor;
@property(copy, nonatomic) NSString *title; // @synthesize title=_title;

@end

