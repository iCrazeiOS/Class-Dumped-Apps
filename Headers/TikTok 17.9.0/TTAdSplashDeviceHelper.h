//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface TTAdSplashDeviceHelper : NSObject
{
}

+ (double)deviceChangeSize:(double)arg1;
+ (double)scaleFontWithScreenWidth:(double)arg1;
+ (double)newFontSize:(double)arg1;
+ (double)newPadding:(double)arg1;
+ (struct CGSize)windowSize;
+ (id)mainWindow;
+ (unsigned long long)currentSplitScreenWithSize:(struct CGSize)arg1;
+ (id)resolutionString;
+ (struct CGSize)resolution;
+ (double)screenScale;
+ (double)ssOnePixel;
+ (id)currentLanguage;
+ (float)OSVersionNumber;
+ (id)idfvString;
+ (id)idfaString;
+ (_Bool)judge480Screen;
+ (_Bool)judge568Screen;
+ (_Bool)judge667Screen;
+ (_Bool)judge736Screen;
+ (_Bool)judge812Screen;
+ (_Bool)judge896Screen;
+ (_Bool)judgePadDevice;
+ (unsigned long long)getDeviceType;
+ (_Bool)isJailBroken;
+ (_Bool)isIpadProDevice;
+ (_Bool)isScreenWidthLarge320;
+ (_Bool)isPadDevice;
+ (_Bool)isAlienScreenDevice;
+ (_Bool)is896Screen;
+ (_Bool)is812Screen;
+ (_Bool)is736Screen;
+ (_Bool)is667Screen;
+ (_Bool)is568Screen;
+ (_Bool)is480Screen;
+ (id)platform;
+ (id)getSysInfoByName:(char *)arg1;
+ (id)platformString;
+ (id)platformName;
+ (long long)getFreeDiskSpace;
+ (long long)getTotalDiskSpace;
+ (id)appLaunchScreenViewWithFrame:(struct CGRect)arg1;
+ (id)appLaunchImage;

@end

