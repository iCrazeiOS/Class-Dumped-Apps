//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class HTSLiveUser;

@interface HTSLiveLinkMicMethod_Contributor : GPBMessage
{
    _Bool _rtcJoinChannel;
}

+ (id)descriptor;
@property(nonatomic) _Bool rtcJoinChannel; // @synthesize rtcJoinChannel=_rtcJoinChannel;

// Remaining properties
@property(nonatomic) _Bool hasUser; // @dynamic hasUser;
@property(nonatomic) long long rank; // @dynamic rank;
@property(nonatomic) long long score; // @dynamic score;
@property(retain, nonatomic) HTSLiveUser *user; // @dynamic user;
@property(nonatomic) long long userId; // @dynamic userId;

@end

