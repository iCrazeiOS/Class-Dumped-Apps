//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray;
@protocol OS_dispatch_queue;

@interface TTTrackerProxy : NSObject
{
    NSObject<OS_dispatch_queue> *_processingQueue;
    NSArray *_blockListv1;
    NSArray *_blockListv3;
    long long _connection;
    long long _throttleSendingTimes;
    long long _successCount;
    long long _maxAllowCount;
    long long _lastBatchTime;
    long long _batchInterval;
    long long _lastDescendTime;
}

+ (id)originDataToStringData:(id)arg1 isV3Format:(_Bool)arg2;
+ (void)setReportBlock:(CDUnknownBlockType)arg1;
+ (_Bool)needEncrypt;
+ (id)generatedPostParamsWithTrack:(id)arg1;
+ (id)generatedHeaderFieldWithPolicy:(long long)arg1;
+ (id)modifiedUploadURLStringWithPolicy:(long long)arg1;
+ (id)sharedProxy;
@property(nonatomic) long long lastDescendTime; // @synthesize lastDescendTime=_lastDescendTime;
@property(nonatomic) long long batchInterval; // @synthesize batchInterval=_batchInterval;
@property(nonatomic) long long lastBatchTime; // @synthesize lastBatchTime=_lastBatchTime;
@property(nonatomic) long long maxAllowCount; // @synthesize maxAllowCount=_maxAllowCount;
@property(nonatomic) long long successCount; // @synthesize successCount=_successCount;
@property(nonatomic) long long throttleSendingTimes; // @synthesize throttleSendingTimes=_throttleSendingTimes;
@property(nonatomic) long long connection; // @synthesize connection=_connection;
@property(copy) NSArray *blockListv3; // @synthesize blockListv3=_blockListv3;
@property(copy) NSArray *blockListv1; // @synthesize blockListv1=_blockListv1;
- (void).cxx_destruct;
- (id)workingQueue;
- (void)clearAllEventData;
- (_Bool)checkThresholdExceed;
- (void)startAscending;
- (void)startDescending;
- (void)modifyThresholdWithCode:(long long)arg1;
- (void)actualSendTrack:(id)arg1 trackerIDs:(id)arg2 v3TrackIDs:(id)arg3 URLString:(id)arg4 headerField:(id)arg5 postParams:(id)arg6 needEncrypt:(_Bool)arg7 policy:(long long)arg8;
- (id)synchronizedRequstForURL:(id)arg1 postParams:(id)arg2;
- (void)sendImmediateEvent:(id)arg1 callback:(CDUnknownBlockType)arg2;
- (void)startSendTrack:(id)arg1 trackIDs:(id)arg2 v3TrackIDs:(id)arg3 policy:(long long)arg4;
- (_Bool)saveData:(id)arg1 isV3Format:(_Bool)arg2 error:(id *)arg3;
- (void)_startHandleTrack:(id)arg1 policy:(long long)arg2 isV3Format:(_Bool)arg3 eventIndex:(id)arg4;
- (void)startHandleTrack:(id)arg1 policy:(long long)arg2 isV3Format:(_Bool)arg3 eventIndex:(id)arg4;
- (id)keyForTrackType:(long long)arg1;
- (long long)connectionType;
- (void)connectionChanged:(id)arg1;
- (void)dealloc;
- (id)init;
- (void)readInterval;

@end

