//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GIDCallbackQueue.h"

@class GIDGTMOAuth2Authentication, NSError, NSString;

@interface GIDAuthFlow : GIDCallbackQueue
{
    GIDGTMOAuth2Authentication *_auth;
    NSError *_error;
    NSString *_emmSupport;
}

@property(copy, nonatomic) NSString *emmSupport; // @synthesize emmSupport=_emmSupport;
@property(retain, nonatomic) NSError *error; // @synthesize error=_error;
@property(retain, nonatomic) GIDGTMOAuth2Authentication *auth; // @synthesize auth=_auth;
- (void).cxx_destruct;

@end

