//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "AWEListDataController.h"

#import "AWEMusicListDataControllerProtocol-Protocol.h"

@class NSNumber, NSString;

@interface AWEMusicAwemeListDataController : AWEListDataController <AWEMusicListDataControllerProtocol>
{
    NSNumber *_offset;
    NSString *_musicID;
}

- (void).cxx_destruct;
@property(copy, nonatomic) NSString *musicID; // @synthesize musicID=_musicID;
@property(copy, nonatomic) NSNumber *offset; // @synthesize offset=_offset;
- (id)p_subArrayWithIndex:(long long)arg1 count:(long long)arg2 dataSource:(id)arg3;
- (void)preloadWithDataSource:(id)arg1;
- (void)resetData;
- (void)fetchWithOriginIndex:(long long)arg1 count:(long long)arg2 completion:(CDUnknownBlockType)arg3;
- (void)fetchWithCount:(long long)arg1 type:(long long)arg2 completion:(CDUnknownBlockType)arg3;
- (void)loadMoreWithFilteredCompletion:(CDUnknownBlockType)arg1;
- (void)refreshWithCompletion:(CDUnknownBlockType)arg1;
- (void)initFetchWithCompletion:(CDUnknownBlockType)arg1;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

