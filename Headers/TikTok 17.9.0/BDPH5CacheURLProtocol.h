//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <CFNetwork/NSURLProtocol.h>

#import "NSURLSessionDataDelegate-Protocol.h"

@class NSHTTPURLResponse, NSMutableArray, NSMutableData, NSString, NSThread, NSURLSessionTask;

@interface BDPH5CacheURLProtocol : NSURLProtocol <NSURLSessionDataDelegate>
{
    NSURLSessionTask *_task;
    NSThread *_clientThread;
    NSMutableArray *_modes;
    NSMutableData *_data;
    NSHTTPURLResponse *_httpResponse;
    NSString *_filePath;
}

+ (id)sharedDemux;
+ (id)canonicalRequestForRequest:(id)arg1;
+ (_Bool)canInitWithRequest:(id)arg1;
@property(copy, nonatomic) NSString *filePath; // @synthesize filePath=_filePath;
@property(retain, nonatomic) NSHTTPURLResponse *httpResponse; // @synthesize httpResponse=_httpResponse;
@property(retain, nonatomic) NSMutableData *data; // @synthesize data=_data;
@property(retain) NSMutableArray *modes; // @synthesize modes=_modes;
@property(retain) NSThread *clientThread; // @synthesize clientThread=_clientThread;
@property(nonatomic) __weak NSURLSessionTask *task; // @synthesize task=_task;
- (void).cxx_destruct;
- (id)filePathWithResponse:(id)arg1;
- (void)unregisterScheme;
- (void)callBlock:(CDUnknownBlockType)arg1;
- (void)URLSession:(id)arg1 task:(id)arg2 didCompleteWithError:(id)arg3;
- (void)URLSession:(id)arg1 task:(id)arg2 willPerformHTTPRedirection:(id)arg3 newRequest:(id)arg4 completionHandler:(CDUnknownBlockType)arg5;
- (void)URLSession:(id)arg1 dataTask:(id)arg2 didReceiveData:(id)arg3;
- (void)URLSession:(id)arg1 dataTask:(id)arg2 didReceiveResponse:(id)arg3 completionHandler:(CDUnknownBlockType)arg4;
- (void)handleError:(id)arg1;
- (void)handleData:(id)arg1;
- (void)handleResponse:(id)arg1;
- (void)stopLoading;
- (void)startLoading;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

