//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GTLRObject.h"

@class GTLRPagespeedonline_PagespeedApiImageV2_PageRect, NSNumber, NSString;

@interface GTLRPagespeedonline_PagespeedApiImageV2 : GTLRObject
{
}

+ (id)propertyToJSONKeyMap;

// Remaining properties
@property(copy, nonatomic) NSString *data; // @dynamic data;
@property(retain, nonatomic) NSNumber *height; // @dynamic height;
@property(copy, nonatomic) NSString *key; // @dynamic key;
@property(copy, nonatomic) NSString *mimeType; // @dynamic mimeType;
@property(retain, nonatomic) GTLRPagespeedonline_PagespeedApiImageV2_PageRect *pageRect; // @dynamic pageRect;
@property(retain, nonatomic) NSNumber *width; // @dynamic width;

@end

