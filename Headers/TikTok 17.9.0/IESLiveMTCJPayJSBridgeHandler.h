//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "IESLiveJSBridgeHandlerProtocol-Protocol.h"

@class NSString;
@protocol IESLiveCJPayService;

@interface IESLiveMTCJPayJSBridgeHandler : NSObject <IESLiveJSBridgeHandlerProtocol>
{
    id <IESLiveCJPayService> _cjpayService;
}

- (void).cxx_destruct;
@property(retain, nonatomic) id <IESLiveCJPayService> cjpayService; // @synthesize cjpayService=_cjpayService;
- (void)registerCJPayH5DeskWithBridge:(id)arg1;
- (void)registerAuthYapilaWithBridge:(id)arg1;
- (void)registerHandlerWithBridge:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

