//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "AWEAppAwemeSettingMessage-Protocol.h"
#import "AWEUserMessage-Protocol.h"

@class NSString;

@interface AWEAppStudioSettingManager : NSObject <AWEAppAwemeSettingMessage, AWEUserMessage>
{
}

+ (_Bool)enableSensitiveWordsCheck;
+ (double)sensitiveWordsCheckRequestTimeout;
+ (_Bool)postVideoCanBindHotSpot;
+ (id)mainPlusButtonConfigModel;
+ (_Bool)enableNewStylePublishLiveRecord;
+ (_Bool)fastPublishNewYearVideo;
+ (id)specialPlusModel;
+ (id)superEntranceModel;
+ (id)effectJsonConfig;
+ (id)tcmNameByContry;
+ (id)adTCMSchemaURL;
+ (long long)prePostCheckFreqLimit;
+ (long long)prePostCheckType;
+ (_Bool)fastImportPassThroughWithoutCheckRotation;
+ (long long)localVideoCacheMaxLength;
+ (long long)localVideoCacheMaxAge;
+ (id)captureAuthorizationHelpURL;
+ (id)freeFlowCardStickerUrl;
+ (double)identificationAsMaleThreshold;
+ (long long)maxFansCount;
+ (id)effectColors;
+ (long long)rearCameraStabilizationMode;
+ (long long)frontCameraStabilizationMode;
+ (_Bool)enablePostDownloadSetting;
+ (_Bool)enableBeautifyEffectsFromPlatform;
+ (_Bool)enableHQVFrame;
+ (id)aiRecommendMusicListDefaultURLLists;
+ (id)aiRecommendMusicListDefaultURI;
+ (long long)aiRecommendMusicImageCount;
+ (_Bool)forbidStoryLocalWatermark;
+ (long long)textStickerMaxCount;
+ (long long)infoStickerMaxCount;
+ (id)goodsOrderShareIntroH5URL;
+ (_Bool)enableWatermarkBackground;
+ (id)publishLinkAdURL;
+ (double)storyPictureDuration;
+ (_Bool)showButtonSyncToInstagramStoryInPublishPage;
+ (_Bool)showButtonSyncToInstagramInPublishPage;
+ (_Bool)forbidVoiceChangeButtonOnEditPage;
+ (_Bool)enableLargeHandDetectModel;
+ (_Bool)enableLargeMattingDetectModel;
+ (long long)createRecordEntry;
+ (id)shootTutorialLink;
+ (id)videoUploadBitrate;
+ (id)videoUploadSize;
+ (id)videoRecordBitrate;
+ (id)videoRecordSize;
+ (double)httpRetryInterval;
+ (long long)httpRetryCount;
+ (_Bool)useTTNetForTTFileUploadClient;
+ (_Bool)useTTEffectPlatformSDK;
+ (_Bool)closeUploadOriginanlFrames;
+ (_Bool)uploadOriginanlAudioTrack;
+ (double)videoCompose;
+ (double)videoCommit;
+ (id)sharedInstance;
- (void)didFinishLogout;
- (void)awemeSettingDidChange;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

