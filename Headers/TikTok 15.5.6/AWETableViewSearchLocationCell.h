//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UITableViewCell.h>

@class UIImageView, UILabel, UIView;

@interface AWETableViewSearchLocationCell : UITableViewCell
{
    UILabel *_locationLabel;
    UILabel *_detailLabel;
    UILabel *_distanceLabel;
    UIView *_separateLine;
    UIImageView *_iconImageView;
}

+ (id)identifier;
@property(retain, nonatomic) UIImageView *iconImageView; // @synthesize iconImageView=_iconImageView;
@property(retain, nonatomic) UIView *separateLine; // @synthesize separateLine=_separateLine;
@property(retain, nonatomic) UILabel *distanceLabel; // @synthesize distanceLabel=_distanceLabel;
@property(retain, nonatomic) UILabel *detailLabel; // @synthesize detailLabel=_detailLabel;
@property(retain, nonatomic) UILabel *locationLabel; // @synthesize locationLabel=_locationLabel;
- (void).cxx_destruct;
- (void)setHighlightedPOILabel:(id)arg1 text:(id)arg2 keyword:(id)arg3 color:(id)arg4;
- (void)configWithItem:(id)arg1 searchKeyword:(id)arg2 hideDistanceWhenSearching:(_Bool)arg3;
- (void)p_addSubviews;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

