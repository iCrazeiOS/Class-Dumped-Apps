//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "AWESearchChallengeViewController.h"

#import "AWEGeneralSearchCellControllerProtocol-Protocol.h"

@class NSDictionary, NSString;

@interface AWESearchChallengeSingleCellController : AWESearchChallengeViewController <AWEGeneralSearchCellControllerProtocol>
{
    unsigned long long _type;
    NSDictionary *_logPassback;
}

@property(copy, nonatomic) NSDictionary *logPassback; // @synthesize logPassback=_logPassback;
@property(nonatomic) unsigned long long type; // @synthesize type=_type;
- (void).cxx_destruct;
@property(copy, nonatomic) NSString *enterFrom;
- (void)footerViewTapped;
- (void)refreshWithRecommendData:(id)arg1 keyword:(id)arg2;
- (void)refreshWithData:(id)arg1 keyword:(id)arg2 hasMore:(_Bool)arg3;
- (void)viewDidLoad;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

