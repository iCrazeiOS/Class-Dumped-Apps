//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "AWEFeedCountManagerProtocol-Protocol.h"

@class NSMutableArray, NSString;

@interface AWEFeedCountManager : NSObject <AWEFeedCountManagerProtocol>
{
    long long _feedCount;
    NSMutableArray *_showed;
}

+ (id)dateFormatter;
+ (id)sharedInstance;
@property(retain, nonatomic) NSMutableArray *showed; // @synthesize showed=_showed;
@property(nonatomic) long long feedCount; // @synthesize feedCount=_feedCount;
- (void).cxx_destruct;
@property(readonly, nonatomic) long long maxFeedCount;
- (void)resetFeedCount;
- (void)didFinishLogout;
- (void)didFinishLogin;
- (_Bool)hasShowedForModel:(id)arg1;
- (void)showForModel:(id)arg1;
- (id)init;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

