//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface PayOrderStruct : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) _Bool hasMoney; // @dynamic hasMoney;
@property(nonatomic) _Bool hasOrderId; // @dynamic hasOrderId;
@property(nonatomic) _Bool hasPayChannel; // @dynamic hasPayChannel;
@property(nonatomic) _Bool hasStatus; // @dynamic hasStatus;
@property(nonatomic) _Bool hasTime; // @dynamic hasTime;
@property(nonatomic) int money; // @dynamic money;
@property(copy, nonatomic) NSString *orderId; // @dynamic orderId;
@property(copy, nonatomic) NSString *payChannel; // @dynamic payChannel;
@property(copy, nonatomic) NSString *status; // @dynamic status;
@property(nonatomic) int time; // @dynamic time;

@end

