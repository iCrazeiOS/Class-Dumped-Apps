//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WAPaymentManager.h"

@class WAPaymentUPIConsumerRequester, WAPaymentUPIFactory, WAPaymentUPIFieldStatsManager, WAPaymentUPINameDirectory, WAPaymentUPIPSPConfiguration, WARepeatingBlockTask;

@interface WAPaymentUPIManager : WAPaymentManager
{
    WAPaymentUPIPSPConfiguration *_pspConfiguration;
    WARepeatingBlockTask *_taskToSyncBlockedHandles;
    WAPaymentUPINameDirectory *_paymentUPINameDirectory;
    WAPaymentUPIFactory *_upiFactory;
}

+ (void)resetUPITokenAndListKeysState;
+ (void)saveUPIToken:(id)arg1;
+ (id)UPIToken;
+ (void)saveUPIListKeys:(id)arg1;
+ (id)UPIListKeys;
+ (void)resetUPIHandleData;
+ (void)saveFBHandleId:(id)arg1;
+ (id)fbHandleId;
+ (void)saveUPIHandle:(id)arg1;
+ (id)UPIHandle;
+ (_Bool)hasUserDefaultsValueExpiredForDateKey:(id)arg1 expirationInterval:(double)arg2;
+ (void)setSandboxEnabled:(_Bool)arg1;
+ (id)sharedManager;
+ (void)resetReceivedIntentFromJIDs;
+ (void)resetSentIntentToJIDs;
- (void).cxx_destruct;
- (void)cancelSyncBlockedHandlesTask;
- (void)scheduleSyncBlockedHandlesTask;
- (id)syncBlockedHandlesTask;
- (void)cacheUnblockedUPIHandleState:(id)arg1;
- (void)cacheBlockedUPIHandleState:(id)arg1;
- (id)cachedBlockedUPIHandlesHash;
- (id)performBlockedHandleHash:(id)arg1;
- (id)cachedBlockedUPIHandles;
- (void)resetUPIPSPConfiguration;
- (void)activatePSPConfiguration:(id)arg1;
@property(readonly, nonatomic) WAPaymentUPIPSPConfiguration *activePSPConfiguration;
- (void)updateCollectRequestTransactionWithId:(id)arg1 toP2PStatusWithBlock:(CDUnknownBlockType)arg2;
- (void)updateUPIMpinSetState:(_Bool)arg1 account:(id)arg2;
- (void)updateUPIMpinSetState:(_Bool)arg1 accountId:(id)arg2;
- (void)updateUPIReceiverInfoForUserJID:(id)arg1 withBlock:(CDUnknownBlockType)arg2 save:(_Bool)arg3;
- (id)upiReceiverInfoForUserJID:(id)arg1;
- (id)defaultUPIPaymentAccount;
- (id)upiAccounts;
- (id)upiAccountWithId:(id)arg1;
- (id)addOutgoingCollectRequestWithCLTransactionId:(id)arg1 amount_1000:(long long)arg2 currency:(id)arg3 receiverJID:(id)arg4 senderHandle:(id)arg5 note:(id)arg6;
- (void)performUpdatesForPotentialDatabaseRebuild;
- (void)cancelBackgroundTasksToSyncDatabase;
- (void)scheduleBackgroundTasksToSyncDatabase;
- (_Bool)hasPaymentAccount;
- (void)fullyResetStateAndResetTOS:(_Bool)arg1 resetWAPIN:(_Bool)arg2;
- (id)factory;
- (id)paymentNameDirectory;
- (id)createPaymentFieldStatsManager;
- (id)createPaymentRequester;
@property(readonly, nonatomic) WAPaymentUPIFieldStatsManager *upiFieldStatsManager;
@property(readonly, nonatomic) WAPaymentUPIConsumerRequester *paymentUPIRequester;
- (id)sendUPIUserSetupNotificationForJID:(id)arg1;
- (id)sendUPIIntentToSendNotificationForJID:(id)arg1;
- (void)sendUPIUserSetupNotifications;
- (void)processIncomingUserSetUpNotification:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (void)processIncomingIntentToSendNotification:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (void)registerXMPPNotificationHandler;
- (void)removeReceivedIntentFromJID:(id)arg1;
- (void)storeReceivedIntentFromJID:(id)arg1;
- (id)receivedIntentFromJIDStrings;
- (id)receivedIntentFromJIDs;
- (void)removeSentIntentToJID:(id)arg1;
- (void)storeSentIntentToJID:(id)arg1;
- (id)sentIntentToJIDStrings;
- (id)sentIntentToJIDs;
- (void)scheduleSyncTransactionTask;
- (void)syncMethodsFromResponse:(id)arg1;
- (void)syncUPIHandlesFromXMPPResponse:(id)arg1;
- (void)persistBlockedUPIHandlesCache:(id)arg1;
- (void)reallyDeclineIncomingCollectRequest:(id)arg1 fromViewController:(id)arg2;
- (void)declineIncomingCollectRequestForTransaction:(id)arg1 fromViewController:(id)arg2;
- (id)requestIdentifierForOngoingAccountRequest:(id)arg1;
- (_Bool)shouldShowLoadingIndicatorForTransaction:(id)arg1;

@end

