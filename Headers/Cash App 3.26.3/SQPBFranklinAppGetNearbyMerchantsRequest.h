//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString, SQPBFranklinAppGetNearbyMerchantsRequest_CurrentLocation, SQPBFranklinCommonRequestContext;

@interface SQPBFranklinAppGetNearbyMerchantsRequest : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(copy, nonatomic) NSString *categoryId; // @dynamic categoryId;
@property(retain, nonatomic) SQPBFranklinAppGetNearbyMerchantsRequest_CurrentLocation *currentLocation; // @dynamic currentLocation;
@property(nonatomic) _Bool hasCategoryId; // @dynamic hasCategoryId;
@property(nonatomic) _Bool hasCurrentLocation; // @dynamic hasCurrentLocation;
@property(nonatomic) _Bool hasRequestContext; // @dynamic hasRequestContext;
@property(retain, nonatomic) SQPBFranklinCommonRequestContext *requestContext; // @dynamic requestContext;

@end

