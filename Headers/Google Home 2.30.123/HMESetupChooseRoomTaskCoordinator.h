//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "HMEModelObserver-Protocol.h"
#import "HMESetupChooseRoomTaskCoordinator-Protocol.h"
#import "HMESetupCustomRoomViewControllerDelegate-Protocol.h"

@class HMEModel, HMESetupChooseRoomViewController, HMESetupCustomRoomViewController, HMESetupNavigationController, NSString;
@protocol HMESetupChooseRoomTaskCoordinatorDelegate, HMESpaceData;

@interface HMESetupChooseRoomTaskCoordinator : NSObject <HMEModelObserver, HMESetupCustomRoomViewControllerDelegate, HMESetupChooseRoomTaskCoordinator>
{
    id <HMESetupChooseRoomTaskCoordinatorDelegate> _delegate;
    id <HMESpaceData> _spaceData;
    HMESetupNavigationController *_navigationController;
    NSString *_structureID;
    NSString *_deviceTypeName;
    HMEModel *_spaceTypesModel;
    HMEModel *_existingRoomsModel;
    HMESetupChooseRoomViewController *_chooseRoomViewController;
    HMESetupCustomRoomViewController *_customRoomViewController;
    unsigned long long _roomSelectionPriority;
}

+ (CDUnknownBlockType)indoorPrioritizationComparator;
+ (CDUnknownBlockType)outdoorPrioritizationComparator;
- (void).cxx_destruct;
@property(nonatomic) unsigned long long roomSelectionPriority; // @synthesize roomSelectionPriority=_roomSelectionPriority;
@property(retain, nonatomic) HMESetupCustomRoomViewController *customRoomViewController; // @synthesize customRoomViewController=_customRoomViewController;
@property(retain, nonatomic) HMESetupChooseRoomViewController *chooseRoomViewController; // @synthesize chooseRoomViewController=_chooseRoomViewController;
@property(retain, nonatomic) HMEModel *existingRoomsModel; // @synthesize existingRoomsModel=_existingRoomsModel;
@property(retain, nonatomic) HMEModel *spaceTypesModel; // @synthesize spaceTypesModel=_spaceTypesModel;
@property(copy, nonatomic) NSString *deviceTypeName; // @synthesize deviceTypeName=_deviceTypeName;
@property(copy, nonatomic) NSString *structureID; // @synthesize structureID=_structureID;
@property(retain, nonatomic) HMESetupNavigationController *navigationController; // @synthesize navigationController=_navigationController;
@property(readonly, nonatomic) id <HMESpaceData> spaceData; // @synthesize spaceData=_spaceData;
@property(nonatomic) __weak id <HMESetupChooseRoomTaskCoordinatorDelegate> delegate; // @synthesize delegate=_delegate;
- (void)didRequestBackFromCustomRoom;
- (void)refreshChooseRoomContent;
- (id)errorMessageForCustomRoomName:(id)arg1 inCustomRoomViewController:(id)arg2;
- (void)modelDidChange:(id)arg1;
- (void)didTapBack;
- (void)didEnterCustomRoom;
- (void)didChooseRoom;
- (void)presentCustomRoomWithDraft:(id)arg1;
- (void)startInNavigationController:(id)arg1 structureID:(id)arg2 deviceTypeName:(id)arg3 headerCopyProvider:(id)arg4 defaultRoomDraft:(id)arg5 options:(long long)arg6 roomSelectionPriority:(unsigned long long)arg7;
- (id)initWithSpaceData:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

