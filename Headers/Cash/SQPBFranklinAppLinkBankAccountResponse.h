//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class SQPBFranklinApiInstrument, SQPBFranklinCommonBankAccount, SQPBFranklinCommonResponseContext;

@interface SQPBFranklinAppLinkBankAccountResponse : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) SQPBFranklinCommonBankAccount *bankAccount; // @dynamic bankAccount;
@property(nonatomic) _Bool hasBankAccount; // @dynamic hasBankAccount;
@property(nonatomic) _Bool hasInstrument; // @dynamic hasInstrument;
@property(nonatomic) _Bool hasResponseContext; // @dynamic hasResponseContext;
@property(retain, nonatomic) SQPBFranklinApiInstrument *instrument; // @dynamic instrument;
@property(retain, nonatomic) SQPBFranklinCommonResponseContext *responseContext; // @dynamic responseContext;

@end

