//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <PSPDFKitUI/NSObject-Protocol.h>

@class NSSet, PSPDFDocumentView, PSPDFDocumentViewLayout, PSPDFDocumentViewScrollView, PSPDFReusableObjectCache;
@protocol PSPDFDocumentViewCoordinatorDelegate;

@protocol PSPDFDocumentViewCoordinator <NSObject>
@property(readonly, nonatomic) NSSet *visibleSpreadViews;
@property(readonly, nonatomic) NSSet *spreadViews;
@property(readonly, nonatomic) NSSet *zoomViews;
@property(nonatomic) __weak id <PSPDFDocumentViewCoordinatorDelegate> delegate;
- (void)scrollToSpreadAtIndex:(long long)arg1 scrollPosition:(long long)arg2 animated:(_Bool)arg3;
- (double)calculateContinuousSpreadIndex;
- (void)setContinuousSpreadIndex:(double)arg1 animated:(_Bool)arg2;
- (void)didMoveToWindow;
- (void)invalidateConfiguration;
- (void)updateForBoundsChange;
- (void)setupViewHierarchyInScrollView:(PSPDFDocumentViewScrollView *)arg1 withLayout:(PSPDFDocumentViewLayout *)arg2;
- (id)initWithDocumentView:(PSPDFDocumentView *)arg1 pageViewCache:(PSPDFReusableObjectCache *)arg2;
@end

