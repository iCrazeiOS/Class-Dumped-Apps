//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@protocol DBTransportClient;

@interface DBCLOUDDOCSUserAuthRoutes : NSObject
{
    id <DBTransportClient> _client;
}

@property(readonly, nonatomic) id <DBTransportClient> client; // @synthesize client=_client;
- (void).cxx_destruct;
- (id)teamSettingsGet;
- (id)getPointerUrl:(id)arg1;
- (id)create:(id)arg1 toPath:(id)arg2 title:(id)arg3 cloudDocType:(id)arg4 pointerUrl:(id)arg5;
- (id)create;
- (id)authorize:(id)arg1 providerAppId:(id)arg2;
- (id)authorize;
- (id)init:(id)arg1;

@end

