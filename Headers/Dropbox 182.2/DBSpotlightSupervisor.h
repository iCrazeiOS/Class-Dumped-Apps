//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "CSSearchableIndexDelegate-Protocol.h"

@class DBSpotlightDatabase, NSString;

@interface DBSpotlightSupervisor : NSObject <CSSearchableIndexDelegate>
{
    DBSpotlightDatabase *_spotlightDatabase;
}

+ (void)db_reindexWithSearchableIndex:(id)arg1 items:(id)arg2 acknowledgementHandler:(CDUnknownBlockType)arg3;
+ (id)db_searchableItemsFromSpotlightItems:(id)arg1;
- (void).cxx_destruct;
- (void)searchableIndex:(id)arg1 reindexSearchableItemsWithIdentifiers:(id)arg2 acknowledgementHandler:(CDUnknownBlockType)arg3;
- (void)searchableIndex:(id)arg1 reindexAllSearchableItemsWithAcknowledgementHandler:(CDUnknownBlockType)arg2;
- (id)initWithSpotlightDatabase:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

