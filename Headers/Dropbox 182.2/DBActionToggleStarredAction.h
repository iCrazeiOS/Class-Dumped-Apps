//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "DBFileAction.h"

@class DBObserverHandle;

@interface DBActionToggleStarredAction : DBFileAction
{
    DBObserverHandle *_starredObserver;
}

+ (void)db_doActionWithoutInteractionForFileEntry:(id)arg1 modalManager:(id)arg2 completion:(CDUnknownBlockType)arg3;
+ (_Bool)shouldDisplayActionForEntries:(id)arg1 atLocation:(long long)arg2;
- (void).cxx_destruct;
- (void)db_updateUIToState:(_Bool)arg1;
- (void)db_updateToCurrentStarredState;
- (void)db_startObservingStarredTracker:(id)arg1;
- (void)performActionWithSender:(id)arg1;
- (void)dealloc;
- (id)initWithActionId:(long long)arg1 actionsController:(id)arg2 location:(long long)arg3;

@end

