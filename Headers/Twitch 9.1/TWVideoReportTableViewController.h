//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TwitchCoreUI/TWBaseTableViewController.h>

#import "UITextViewDelegate-Protocol.h"

@class NSIndexPath, NSString, UIBarButtonItem;

__attribute__((visibility("hidden")))
@interface TWVideoReportTableViewController : TWBaseTableViewController <UITextViewDelegate>
{
    NSIndexPath *_workingIndexPath;
    UIBarButtonItem *_applyButton;
}

- (void).cxx_destruct;
@property(retain, nonatomic) UIBarButtonItem *applyButton; // @synthesize applyButton=_applyButton;
- (void)sendReport;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (_Bool)shouldAutorotate;
- (void)viewDidLoad;
- (id)initWithThemeManager:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

