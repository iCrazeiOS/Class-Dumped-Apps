//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TwitchCoreUI/TWBaseViewController.h>

__attribute__((visibility("hidden")))
@interface _TtC6Twitch36TroubleLoginEmailPhoneViewController : TWBaseViewController
{
    // Error parsing type: , name: scrollableStackContainerView
    // Error parsing type: , name: errorView
    // Error parsing type: , name: titleLabel
    // Error parsing type: , name: subtitleLabel
    // Error parsing type: , name: spacer
    // Error parsing type: , name: emailPhoneField
    // Error parsing type: , name: $__lazy_storage_$_linkableProblemsTextView
    // Error parsing type: , name: continueButton
    // Error parsing type: , name: passportError
    // Error parsing type: , name: isPhoneEntry
    // Error parsing type: , name: $__lazy_storage_$_togglePhoneSignupButton
    // Error parsing type: , name: $__lazy_storage_$_loadingView
    // Error parsing type: , name: requestIsInFlight
}

- (void).cxx_destruct;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2 themeManager:(id)arg3;
- (void)togglePhoneSignupButtonTapped;
- (void)continueToUsernameView;
- (void)continueButtonTapped;
- (void)dismissKeyboard;
- (void)applyTheme:(id)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (id)initWithThemeManager:(id)arg1;

@end

