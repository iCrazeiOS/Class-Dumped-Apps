//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TwitchCoreUI/TWThemeableView.h>

@protocol _TtP6Twitch33NoContentConfigurableViewDelegate_;

__attribute__((visibility("hidden")))
@interface _TtC6Twitch25NoContentConfigurableView : TWThemeableView
{
    // Error parsing type: , name: iconView
    // Error parsing type: , name: titleLabel
    // Error parsing type: , name: descriptionLabel
    // Error parsing type: , name: actionButton
    // Error parsing type: , name: stackView
    // Error parsing type: , name: loadingIndicator
    // Error parsing type: , name: delegate
}

- (void).cxx_destruct;
- (id)initWithFrame:(struct CGRect)arg1 themeManager:(id)arg2;
- (void)applyTheme:(id)arg1;
- (void)actionButtonPressed;
@property(nonatomic) _Bool isLoading;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (id)initWithCoder:(id)arg1;
@property(nonatomic) __weak id <_TtP6Twitch33NoContentConfigurableViewDelegate_> delegate; // @synthesize delegate;

@end

