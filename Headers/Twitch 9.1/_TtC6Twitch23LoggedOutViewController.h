//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TwitchCoreUI/TWBaseViewController.h>

@protocol _TtP6Twitch31LoggedOutViewControllerDelegate_;

__attribute__((visibility("hidden")))
@interface _TtC6Twitch23LoggedOutViewController : TWBaseViewController
{
    // Error parsing type: , name: delegate
    // Error parsing type: , name: areLoginAndSignupButtonsVisible
    // Error parsing type: , name: loginAnalyticsController
    // Error parsing type: , name: loadingIndicator
    // Error parsing type: , name: scrollableStackContainerView
    // Error parsing type: , name: titleLabel
    // Error parsing type: , name: loginButton
    // Error parsing type: , name: signupButton
    // Error parsing type: , name: screenName
}

- (void).cxx_destruct;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2 themeManager:(id)arg3;
- (void)showLoginAndSignupButtons;
- (void)applyTheme:(id)arg1;
@property(nonatomic, readonly) _Bool wantsNavigationBarHidden;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (void)tapSignup;
- (void)tapLogin;
- (id)initWithThemeManager:(id)arg1;
@property(nonatomic) __weak id <_TtP6Twitch31LoggedOutViewControllerDelegate_> delegate; // @synthesize delegate;

@end

