//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@interface _TtC7Scanner26BarcodeScannerScanItemsBar : UIView
{
    // Error parsing type: , name: label
    // Error parsing type: , name: chevronImageView
    // Error parsing type: , name: stackView
    // Error parsing type: , name: stackViewVisible
    // Error parsing type: , name: $__lazy_storage_$_stackViewHeightConstraint
    // Error parsing type: , name: presentationMode
}

- (void).cxx_destruct;
- (void)toggleItemsVisibility;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;

@end

