//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <swiftCore/_TtCs12_SwiftObject.h>

@interface _TtC22ConnectUIV2FeatureImpl23ConnectUIV2FlagsManager : _TtCs12_SwiftObject
{
    // Error parsing type: , name: remoteConfigurationResolver
    // Error parsing type: , name: $__lazy_storage_$_isMiniPickerEnabled
    // Error parsing type: , name: $__lazy_storage_$_isMiniPickerEnabledInRemoteConfig
    // Error parsing type: , name: $__lazy_storage_$_remoteConfigProperties
    // Error parsing type: , name: $__lazy_storage_$_isConnectAttachedDestinationEnabled
    // Error parsing type: , name: $__lazy_storage_$_isConnectDestinationNudgeEnabled
    // Error parsing type: , name: $__lazy_storage_$_shouldShowActiveDeviceInNPB
    // Error parsing type: , name: $__lazy_storage_$_isConnectActiveDeviceInNPBEnabled
    // Error parsing type: , name: $__lazy_storage_$_isCustomDeviceSortingEnabled
    // Error parsing type: , name: $__lazy_storage_$_isFrictionlessJoinEnabled
}

@property(nonatomic) _Bool shouldShowActiveDeviceInNPB;
@property(nonatomic) _Bool isConnectDestinationNudgeEnabled;
@property(nonatomic) _Bool isConnectAttachedDestinationEnabled;

@end

