//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSCopying-Protocol.h"
#import "SPTEncoreTypeStyleBuilder-Protocol.h"

@class UIFont;

@interface SPTEncoreTypeStyle : NSObject <SPTEncoreTypeStyleBuilder, NSCopying>
{
    _Bool _upperCaseLayout;
    UIFont *_font;
    CDStruct_63798c11 _fontSizeRange;
    CDStruct_63798c11 _trackingRange;
    CDStruct_63798c11 _lineHeightRange;
    CDStruct_63798c11 _minimumScaleFactorRange;
}

+ (double)valueFromRange:(CDStruct_d2b197d1)arg1 withScale:(double)arg2;
+ (id)typeStyleWithFont:(id)arg1 fontSizeRange:(CDStruct_d2b197d1)arg2 lineHeightRange:(CDStruct_d2b197d1)arg3 minimumScaleFactorRange:(CDStruct_d2b197d1)arg4 trackingRange:(CDStruct_d2b197d1)arg5 upperCaseLayout:(_Bool)arg6;
+ (id)constrainedBadge;
+ (void)set_constrainedBadge:(id)arg1;
+ (id)_constrainedBadge;
+ (id)badge;
+ (void)set_badge:(id)arg1;
+ (id)_badge;
+ (id)spt_videoLeaveBehindSubtitleStyle;
+ (id)spt_videoLeaveBehindTitleStyle;
+ (id)cello;
+ (id)canon;
+ (id)alto;
+ (id)metronome;
+ (id)minuetBold;
+ (id)minuet;
+ (id)finaleBold;
+ (id)finale;
+ (id)brio;
+ (id)forte;
+ (id)bass;
+ (id)mestoBold;
+ (id)mesto;
+ (id)violaBold;
+ (id)viola;
+ (id)balladBold;
+ (id)ballad;
+ (id)stringAttributesForStyle:(id)arg1;
+ (double)kerningFromTracking:(double)arg1 withFont:(id)arg2;
+ (double)kerningFromLetterSpacing:(double)arg1 withFront:(id)arg2;
+ (double)lineHeightMultipleFromLineHeight:(double)arg1 withFont:(id)arg2;
+ (id)fontFromTypeStyle:(id)arg1 withAccessibilityScale:(double)arg2;
+ (id)paragraphStyleFromStyle:(id)arg1 withAccessibilityScale:(double)arg2 andFont:(id)arg3;
+ (id)stringAttributesForStyle:(id)arg1 withAccessibilityScale:(double)arg2;
+ (id)attributedStringFromText:(id)arg1 withStyle:(id)arg2 andAccessibilityScale:(double)arg3;
+ (id)mergeParagraphStyle:(id)arg1 withParagraphStyle:(id)arg2;
+ (id)mergedStringAttributes:(id)arg1;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool upperCaseLayout; // @synthesize upperCaseLayout=_upperCaseLayout;
@property(readonly, nonatomic) CDStruct_d2b197d1 minimumScaleFactorRange; // @synthesize minimumScaleFactorRange=_minimumScaleFactorRange;
@property(readonly, nonatomic) CDStruct_d2b197d1 lineHeightRange; // @synthesize lineHeightRange=_lineHeightRange;
@property(readonly, nonatomic) CDStruct_d2b197d1 trackingRange; // @synthesize trackingRange=_trackingRange;
@property(readonly, nonatomic) CDStruct_d2b197d1 fontSizeRange; // @synthesize fontSizeRange=_fontSizeRange;
@property(readonly, nonatomic) UIFont *font; // @synthesize font=_font;
- (_Bool)isEqual:(id)arg1;
- (unsigned long long)hash;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)description;
- (id)initWithFont:(id)arg1 fontSizeRange:(CDStruct_d2b197d1)arg2 lineHeightRange:(CDStruct_d2b197d1)arg3 minimumScaleFactorRange:(CDStruct_d2b197d1)arg4 trackingRange:(CDStruct_d2b197d1)arg5 upperCaseLayout:(_Bool)arg6;
@property(readonly, nonatomic) double minimumScaleFactor;
- (id)stringAttributes;
- (id)stringAttributesWithAccessibilityScale:(double)arg1;
- (id)attributedStringFromText:(id)arg1 withAccessibilityScale:(double)arg2;
- (id)withUpperCaseLayout:(_Bool)arg1;
- (id)withMinimumScaleFactor:(double)arg1;
- (id)withMinimumScaleFactorRange:(CDStruct_d2b197d1)arg1;
- (id)withLineHeight:(double)arg1;
- (id)withLineHeightRange:(CDStruct_d2b197d1)arg1;
- (id)withTracking:(double)arg1;
- (id)withTrackingRange:(CDStruct_d2b197d1)arg1;
- (id)withFontSize:(double)arg1;
- (id)withFontSizeRange:(CDStruct_d2b197d1)arg1;
- (id)builder;

@end

