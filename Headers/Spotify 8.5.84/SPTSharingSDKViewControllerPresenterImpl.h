//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "SPTSharingSDKViewControllerPresenter-Protocol.h"

@class NSString, UIViewController;

@interface SPTSharingSDKViewControllerPresenterImpl : NSObject <SPTSharingSDKViewControllerPresenter>
{
    UIViewController *_contextViewController;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) __weak UIViewController *contextViewController; // @synthesize contextViewController=_contextViewController;
- (void)presentViewController:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (id)initWithContextViewController:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

