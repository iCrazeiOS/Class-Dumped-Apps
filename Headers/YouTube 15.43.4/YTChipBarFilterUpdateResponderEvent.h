//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Module_Framework/YTResponderEvent.h>

@class YTIReloadContinuationData;

@interface YTChipBarFilterUpdateResponderEvent : YTResponderEvent
{
    _Bool _isFilterActive;
    YTIReloadContinuationData *_continuation;
}

+ (id)eventWithFirstResponder:(id)arg1 continuation:(id)arg2 isFilterActive:(_Bool)arg3;
- (void).cxx_destruct;
@property(nonatomic) _Bool isFilterActive; // @synthesize isFilterActive=_isFilterActive;
@property(readonly, nonatomic) YTIReloadContinuationData *continuation; // @synthesize continuation=_continuation;
- (id)initWithFirstResponder:(id)arg1 continuation:(id)arg2 isFilterActive:(_Bool)arg3;

@end

