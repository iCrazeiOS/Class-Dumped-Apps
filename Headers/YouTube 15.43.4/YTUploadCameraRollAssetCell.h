//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Module_Framework/YTCollectionViewCell.h>

#import <Module_Framework/YTCollectionViewCellProtocol-Protocol.h>
#import <Module_Framework/YTResponder-Protocol.h>

@class NSString, UIImageView, YTIFormattedString, YTUploadCameraRollAsset, YTVideoThumbnailView;
@protocol YTResponder;

@interface YTUploadCameraRollAssetCell : YTCollectionViewCell <YTCollectionViewCellProtocol, YTResponder>
{
    YTUploadCameraRollAsset *_cameraRollAsset;
    YTVideoThumbnailView *_thumbnailView;
    YTIFormattedString *_durationText;
    UIImageView *_borderImageView;
    UIImageView *_checkmarkImageView;
    UIImageView *_uploadingIconImageView;
    id <YTResponder> _parentResponder;
}

+ (struct CGSize)cellSizeForEntry:(id)arg1 size:(struct CGSize)arg2 gimme:(id)arg3 safeAreaInsets:(struct UIEdgeInsets)arg4;
+ (struct CGSize)cellSizeForEntry:(id)arg1 size:(struct CGSize)arg2;
- (void).cxx_destruct;
@property(nonatomic) __weak id <YTResponder> parentResponder; // @synthesize parentResponder=_parentResponder;
- (void)setThumbnail:(id)arg1 animated:(_Bool)arg2;
- (void)setSelected:(_Bool)arg1;
- (id)entry;
- (void)setEntry:(id)arg1;
- (void)prepareForReuse;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1 customThumbnail:(_Bool)arg2;
- (id)initWithFrame:(struct CGRect)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

