//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "YTFeedFilterChipBarNavigationDelegate-Protocol.h"
#import "YTResponseViewController-Protocol.h"
#import "YTUserPullToRefreshObserver-Protocol.h"
#import "YTVariableHeightHeaderViewControllerAttachedHeader-Protocol.h"

@class GIMMe, NSString, YTIMySubsFilterHeaderRenderer, YTInnerTubeControllerFactory;
@protocol YTDataServices, YTRendererController, YTResponder, YTServices;

@interface YTMySubsFilterHeaderViewController : UIViewController <YTVariableHeightHeaderViewControllerAttachedHeader, YTFeedFilterChipBarNavigationDelegate, YTResponseViewController, YTUserPullToRefreshObserver>
{
    UIViewController<YTRendererController> *_channelFilterViewController;
    UIViewController<YTRendererController> *_chipFilterViewController;
    YTInnerTubeControllerFactory *_controllerFactory;
    id <YTServices> _services;
    id <YTDataServices> _dataServices;
    YTIMySubsFilterHeaderRenderer *_renderer;
    _Bool _filteringByChannel;
    _Bool _filteringByChip;
    _Bool _userPulledToRefresh;
    id <YTResponder> _parentResponder;
    GIMMe *_gimme;
}

- (void).cxx_destruct;
@property(nonatomic) __weak GIMMe *gimme; // @synthesize gimme=_gimme;
@property(readonly, nonatomic) __weak id <YTResponder> parentResponder; // @synthesize parentResponder=_parentResponder;
- (void)loadChipFilterFromModel:(id)arg1;
- (void)loadChannelFilterFromModel:(id)arg1;
- (_Bool)shouldReloadChipFilter;
- (_Bool)shouldReloadChannelFilter;
- (void)userDidPullToRefresh;
- (void)chipBar:(id)arg1 view:(id)arg2 requestsNavigation:(id)arg3;
- (_Bool)scrollToModelMatchingBlock:(CDUnknownBlockType)arg1 animated:(_Bool)arg2;
- (void)willMakeRequest:(id)arg1;
- (void)loadWithModel:(id)arg1;
- (_Bool)isAttachedToPage;
- (void)exitFilterByChannel;
- (void)loadView;
- (void)dealloc;
- (id)initWithParentResponder:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(retain, nonatomic) NSString *overrideStyleContext;
@property(readonly) Class superclass;
@property(nonatomic) _Bool useDefaultModelWhenEmpty;

@end

