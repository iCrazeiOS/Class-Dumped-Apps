//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Module_Framework/GPBMessage.h>

@class NSString;

@interface GRWSKeyValuePair : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) _Bool boolValue; // @dynamic boolValue;
@property(nonatomic) int clientValue; // @dynamic clientValue;
@property(nonatomic) _Bool hasKey; // @dynamic hasKey;
@property(nonatomic) int intValue; // @dynamic intValue;
@property(copy, nonatomic) NSString *key; // @dynamic key;
@property(copy, nonatomic) NSString *stringValue; // @dynamic stringValue;
@property(readonly, nonatomic) int valueTypesOneOfCase; // @dynamic valueTypesOneOfCase;

@end

