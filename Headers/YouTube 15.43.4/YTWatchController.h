//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "MDXAutonavControllerDelegate-Protocol.h"
#import "MDXSessionManagerObserver-Protocol.h"
#import "MDXSessionObserver-Protocol.h"
#import "YCHPlaybackEventsProvider-Protocol.h"
#import "YTActiveVideoProvider-Protocol.h"
#import "YTActiveVideoReportingRendererProvider-Protocol.h"
#import "YTAppOfflineVideoControllerObserver-Protocol.h"
#import "YTAutonavPlayabilityDelegate-Protocol.h"
#import "YTBedtimeReminderControllerDelegate-Protocol.h"
#import "YTClipControllerDelegate-Protocol.h"
#import "YTCompanionRenderingProtocol-Protocol.h"
#import "YTEngagementPanelNavigationControllerEventObserver-Protocol.h"
#import "YTExternalCompanionShelfResponderProvider-Protocol.h"
#import "YTImmersiveRenderingProtocol-Protocol.h"
#import "YTOfflineModeObserver-Protocol.h"
#import "YTOfflineVideoControllerObserver-Protocol.h"
#import "YTOfflineabilityResponderProvider-Protocol.h"
#import "YTPlaybackControllerEventsDelegate-Protocol.h"
#import "YTPlaybackControllerUIDelegate-Protocol.h"
#import "YTPlaybackDebugDataProvider-Protocol.h"
#import "YTPlaylistPanelControllerPlaybackDelegate-Protocol.h"
#import "YTResponder-Protocol.h"
#import "YTShareObserver-Protocol.h"
#import "YTShortsRemixVideoResponderProvider-Protocol.h"
#import "YTSpeedyGResponderProvider-Protocol.h"
#import "YTVELoggingAPI-Protocol.h"
#import "YTWatchBreakControllerDelegate-Protocol.h"
#import "YTWatchControllerProtocol-Protocol.h"
#import "YTWatchNextStateResponderProvider-Protocol.h"
#import "YTYouThereControllerDelegate-Protocol.h"

@class GIMMe, MDXAutonavController, NSError, NSHashTable, NSMutableDictionary, NSString, UIView, UIViewController, YTAccountLinkingController, YTClipController, YTDelayedWatchNextService, YTICommand, YTIMenuRenderer, YTIVideoReportingSupportedRenderers, YTIWatchNextResponse, YTInnerTubeControllerFactory, YTPlaybackDebugData, YTPlayerResponse, YTPlayerWatchNextOrderingController, YTPlaylistPanelController, YTRestoreController, YTShareEndpointController, YTVideoQualityPromoController, YTWatchNextResultsViewController, YTWatchNextViewController, YTWatchSurveyTriggerController, YTWatchTransition, YTYouThereController;
@protocol MDXServices, YTEngagementPanelNavigationContainerProtocol, YTInceptionControllerProtocol, YTInfoCardContainerProtocol, YTLiveChatContainerProtocol, YTPlaybackController, YTResponder, YTServices, YTWatchControllerDelegate, YTWatchFlowControllerProtocol, YTWatchNextResultsControllerProtocol;

@interface YTWatchController : NSObject <YTAppOfflineVideoControllerObserver, YTAutonavPlayabilityDelegate, YTCompanionRenderingProtocol, YTClipControllerDelegate, YTEngagementPanelNavigationControllerEventObserver, YTExternalCompanionShelfResponderProvider, YTImmersiveRenderingProtocol, YTOfflineModeObserver, YTOfflineVideoControllerObserver, YTPlaybackControllerUIDelegate, YTShareObserver, YTSpeedyGResponderProvider, YTVELoggingAPI, YTWatchBreakControllerDelegate, YTYouThereControllerDelegate, MDXSessionObserver, MDXSessionManagerObserver, YTBedtimeReminderControllerDelegate, MDXAutonavControllerDelegate, YTShortsRemixVideoResponderProvider, YTActiveVideoProvider, YTActiveVideoReportingRendererProvider, YCHPlaybackEventsProvider, YTOfflineabilityResponderProvider, YTPlaybackControllerEventsDelegate, YTPlaybackDebugDataProvider, YTPlaylistPanelControllerPlaybackDelegate, YTResponder, YTWatchControllerProtocol, YTWatchNextStateResponderProvider>
{
    id <YTWatchNextResultsControllerProtocol> _watchNextResultsController;
    YTICommand *_navigationEndpoint;
    int _watchEndpointSource;
    YTPlaybackDebugData *_savedPlaybackDebugData;
    YTPlayerResponse *_savedPlayerResponse;
    _Bool _watchNextResponseLoaded;
    NSHashTable *_offlineabilityObservers;
    NSHashTable *_speedyGObservers;
    NSHashTable *_playbackEventsObservers;
    YTVideoQualityPromoController *_videoQualityPromoController;
    YTIVideoReportingSupportedRenderers *_videoReportingRenderers;
    YTICommand *_addToMenuEndpoint;
    YTICommand *_addToMenuLongPressEndpoint;
    YTIMenuRenderer *_addToMenuRenderer;
    YTWatchSurveyTriggerController *_surveyTriggerController;
    id <YTWatchFlowControllerProtocol> _watchFlowController;
    YTPlayerWatchNextOrderingController *_orderingController;
    YTDelayedWatchNextService *_delayedWatchNextService;
    YTYouThereController *_youThereController;
    YTIWatchNextResponse *_lastInitialWatchNextResponse;
    YTRestoreController *_restoreController;
    YTWatchTransition *_preparedWatchTransition;
    _Bool _videoWasPausedForShareSheet;
    id <YTInceptionControllerProtocol> _inceptionController;
    YTICommand *_watchBreakTriggeredCommand;
    _Bool _isWatchBreakActive;
    YTICommand *_showBedtimeReminderPanelCommand;
    _Bool _isBedtimeReminderActive;
    NSError *_deferredUnplayableError;
    _Bool _playbackHasCompanionAd;
    _Bool _hasUpdatedCompanionAdForCurrentPlayback;
    _Bool _isUiReady;
    MDXAutonavController *_MDXAutonavController;
    YTPlaylistPanelController *_playlistPanelController;
    _Bool _pausedBeforePlayback;
    _Bool _clipPlaybackActive;
    id <YTResponder> _parentResponder;
    GIMMe *_gimme;
    id <YTWatchControllerDelegate> _watchControllerDelegate;
    id <YTServices> _services;
    id <MDXServices> _MDXServices;
    YTInnerTubeControllerFactory *_innerTubeControllerFactory;
    UIViewController<YTPlaybackController> *_playerViewController;
    YTWatchNextViewController *_watchNextViewController;
    YTWatchNextResultsViewController *_descriptionDetailsViewController;
    unsigned long long _watchPlaylistEndpointRequestID;
    YTShareEndpointController *_shareEndpointController;
    YTICommand *_watchTransitionNavigationEndpoint;
    YTICommand *_shareEndpoint;
    YTICommand *_sharePrivateEndpoint;
    id <YTInfoCardContainerProtocol> _watchNextInfoCardContainer;
    id <YTLiveChatContainerProtocol> _watchNextLiveChatContainer;
    id <YTEngagementPanelNavigationContainerProtocol> _engagementPanelNavigationContainer;
    NSMutableDictionary *_inlinedTransitions;
    YTClipController *_clipController;
    YTAccountLinkingController *_accountLinkingController;
}

- (void).cxx_destruct;
@property(retain, nonatomic) YTAccountLinkingController *accountLinkingController; // @synthesize accountLinkingController=_accountLinkingController;
@property(nonatomic, getter=isClipPlaybackActive) _Bool clipPlaybackActive; // @synthesize clipPlaybackActive=_clipPlaybackActive;
@property(retain, nonatomic) YTClipController *clipController; // @synthesize clipController=_clipController;
@property(retain, nonatomic) NSMutableDictionary *inlinedTransitions; // @synthesize inlinedTransitions=_inlinedTransitions;
@property(readonly, nonatomic) id <YTEngagementPanelNavigationContainerProtocol> engagementPanelNavigationContainer; // @synthesize engagementPanelNavigationContainer=_engagementPanelNavigationContainer;
@property(readonly, nonatomic) id <YTLiveChatContainerProtocol> watchNextLiveChatContainer; // @synthesize watchNextLiveChatContainer=_watchNextLiveChatContainer;
@property(readonly, nonatomic) id <YTInfoCardContainerProtocol> watchNextInfoCardContainer; // @synthesize watchNextInfoCardContainer=_watchNextInfoCardContainer;
@property(retain, nonatomic) YTICommand *sharePrivateEndpoint; // @synthesize sharePrivateEndpoint=_sharePrivateEndpoint;
@property(retain, nonatomic) YTICommand *shareEndpoint; // @synthesize shareEndpoint=_shareEndpoint;
@property(retain, nonatomic) YTICommand *watchTransitionNavigationEndpoint; // @synthesize watchTransitionNavigationEndpoint=_watchTransitionNavigationEndpoint;
@property(retain, nonatomic) YTShareEndpointController *shareEndpointController; // @synthesize shareEndpointController=_shareEndpointController;
@property(readonly, nonatomic) unsigned long long watchPlaylistEndpointRequestID; // @synthesize watchPlaylistEndpointRequestID=_watchPlaylistEndpointRequestID;
@property(retain, nonatomic) YTWatchNextResultsViewController *descriptionDetailsViewController; // @synthesize descriptionDetailsViewController=_descriptionDetailsViewController;
@property(readonly, nonatomic) YTWatchNextViewController *watchNextViewController; // @synthesize watchNextViewController=_watchNextViewController;
@property(readonly, nonatomic) UIViewController<YTPlaybackController> *playerViewController; // @synthesize playerViewController=_playerViewController;
@property(readonly, nonatomic) YTInnerTubeControllerFactory *innerTubeControllerFactory; // @synthesize innerTubeControllerFactory=_innerTubeControllerFactory;
@property(readonly, nonatomic) __weak id <MDXServices> MDXServices; // @synthesize MDXServices=_MDXServices;
@property(readonly, nonatomic) __weak id <YTServices> services; // @synthesize services=_services;
@property(nonatomic, getter=isPausedBeforePlayback) _Bool pausedBeforePlayback; // @synthesize pausedBeforePlayback=_pausedBeforePlayback;
@property(nonatomic) __weak id <YTWatchControllerDelegate> watchControllerDelegate; // @synthesize watchControllerDelegate=_watchControllerDelegate;
@property(nonatomic) __weak GIMMe *gimme; // @synthesize gimme=_gimme;
@property(readonly, nonatomic) __weak id <YTResponder> parentResponder; // @synthesize parentResponder=_parentResponder;
- (void)displayFirstMealbarInPlayerResponse:(id)arg1 withTriggerCondition:(int)arg2;
- (id)navigationEndpoint;
- (void)hideEngagementPanelWithPanelIdentifier:(id)arg1;
- (id)engagementPanelIdentifierWithCommand:(id)arg1;
- (_Bool)isPanelIdentifier:(id)arg1 sameAsPanelIdentifierOfCommand:(id)arg2;
- (void)displayError:(id)arg1;
- (void)handleLongPressOnAddToButton:(id)arg1;
- (void)resetCompanionAdElementDataStore;
- (void)updateCompanionAdElementsDataStore;
- (_Bool)isCompanionAdLoaded;
- (void)loadCompanionAd:(id)arg1;
- (id)contentPlayerOverlay;
- (_Bool)ignoreStateChangeForWatchBreak;
- (_Bool)isInlinePlayback;
- (void)expandWatchIfRequired;
- (void)setPlayerViewController:(id)arg1;
- (_Bool)handleCommandResponderEvent:(id)arg1;
- (void)handleWatchNavigationEndpointResponderEvent:(id)arg1;
- (void)handleWatchEndpointResponderEvent:(id)arg1;
- (_Bool)handleNavigationEndpointResponderEvent:(id)arg1;
- (void)notifyOfflineabilityObserversDidChange;
- (void)loadWatchNextInfoCardControllers;
- (void)loadCompanionFromPlayerResponse;
- (_Bool)handleOfflineButtonEvent:(id)arg1;
- (void)notifySpeedyGObservers;
- (void)maybeCreateAndSetYouThereControllerWithPlayerResponse:(id)arg1;
- (void)loadCompanionAd:(id)arg1 layoutID:(id)arg2 interactionLoggingAdsClientData:(id)arg3;
- (_Bool)isPlaybackFinished;
- (double)contentVideoTotalTime;
- (double)contentVideoCurrentTime;
- (void)clipControllerDidFinishPlayingClip:(id)arg1;
- (void)clipController:(id)arg1 didActivateClipWithStartTime:(double)arg2 endTime:(double)arg3;
- (void)attachVEWithResponse:(id)arg1;
- (_Bool)isImmersiveRenderingSurfaceReady;
- (void)clearEngagementPanelWithID:(id)arg1;
- (void)loadEngagementPanels:(id)arg1 onUIReadyCommand:(id)arg2;
- (_Bool)isCompanionRenderingSurfaceReady;
- (void)mediaLayoutDidFinishWithLayoutID:(id)arg1 completeType:(long long)arg2;
- (void)clearCompanionWithLayoutID:(id)arg1;
- (void)startCompanion:(id)arg1 withLayoutID:(id)arg2 interactionLoggingAdsClientData:(id)arg3;
- (_Bool)shouldAutonavToNextVideo;
- (void)maybeAutoplay;
- (id)watchNextServiceForNavEndpoint:(id)arg1;
- (void)createWatchNextViewControllerIfNeeded;
- (void)destroyWatchNextViewController;
- (void)claimPlayerViewController;
- (void)cleanupAccountLinkController;
- (void)setupAccountLinkControllerIfNeededWithPlayerResponse:(id)arg1;
- (void)startClipPlaybackIfClipConfigAvailableForVideo:(id)arg1 withPlaybackData:(id)arg2;
- (_Bool)isMDXActive;
- (void)resetPlayerViewControllerStartPlayback:(_Bool)arg1 latencyLogger:(id)arg2;
- (void)updatePlayerControllerControls;
- (id)CSN;
- (void)setEngagementPanelNavigationContainer:(id)arg1;
- (void)setWatchFlowController:(id)arg1;
- (void)prepareWithWatchNextResponse:(id)arg1;
- (id)currentMediaTimeEndpoint;
- (id)resign;
- (void)applyWatchTransition:(id)arg1 startPlayback:(_Bool)arg2;
- (_Bool)shouldExitFullScreenOnFinish;
- (void)fetchWatchNextForWatchTransition:(id)arg1 startPlayback:(_Bool)arg2;
- (void)prepareForWatchTransition:(id)arg1 startPlayback:(_Bool)arg2;
- (void)startLatencyTickLoggerForTransition:(id)arg1;
- (void)handleDidToggleAutoplayResponderEvent;
- (void)didUpdateAutoplayRenderer;
- (void)autonavController:(id)arg1 didChangeAutoplayEnabled:(_Bool)arg2;
- (void)onHasPreviousNextChangedForMDXSession:(id)arg1 hasPrevious:(_Bool)arg2 hasNext:(_Bool)arg3;
- (void)MDXSession:(id)arg1 playlistDidChangeToListID:(id)arg2 videoIndex:(unsigned long long)arg3 firstVideoID:(id)arg4;
- (void)MDXSessionManager:(id)arg1 willStartSession:(id)arg2;
- (void)offlineModeDidChange;
- (void)shareSheetExitedFullScreen;
- (void)shareSheetEnteredFullScreen;
- (void)exitFullScreenIfRequiredWithCompletion:(CDUnknownBlockType)arg1;
- (void)exitFullScreenIfRequired;
- (id)currentCPNForYouThereController:(id)arg1;
- (id)currentCSN;
- (void)youThereControllerDidTransitionFromState:(long long)arg1 toState:(long long)arg2;
- (void)shouldShowBedtimeReminderAfterVideoEndsDidChange:(_Bool)arg1;
- (void)didHideBedtimeReminderOnBrowseSurfaceWhilePlayerIsCollapsed;
- (void)willShowBedtimeReminderOnBrowseSurfaceWhilePlayerIsCollapsed;
- (void)willShowBedtimeReminderOnWatchSurfaceWithShowBedtimeReminderPanelCommand:(id)arg1;
- (void)watchBreakFrequencyDidChange;
- (void)showWatchBreak;
- (_Bool)isPlaybackVideoPlayingAd;
- (_Bool)isPlaybackVideoPlaying;
- (double)contentVideoMediaTime;
- (void)removePlaybackEventsObserver:(id)arg1;
- (void)addPlaybackEventsObserver:(id)arg1;
- (void)addOfflineabilityChangeObserver:(id)arg1;
- (void)addSpeedyGObserver:(id)arg1;
- (void)unloadWatchNextResponse;
- (_Bool)playlistShuffleEnabledOnLoad;
- (void)setAutoplay:(id)arg1 autonav:(id)arg2 playlistPanel:(id)arg3 playlistUIDelegate:(id)arg4 infoCardContainer:(id)arg5 liveChatContainer:(id)arg6 watchNextResults:(id)arg7 descriptionDetails:(id)arg8 currentEndpoint:(id)arg9 videoReportingRenderers:(id)arg10 addToMenuRenderer:(id)arg11 addToMenuEndpoint:(id)arg12 addToMenuLongPressEndpoint:(id)arg13 shareEndpoint:(id)arg14 watchNextResponse:(id)arg15;
- (_Bool)isExternalCompanionShelfAllowed;
- (void)willCollapseEngagementPanelForPanelIdentifier:(id)arg1;
- (void)willCollapseEngagementPanelsNavigationView;
- (void)engagementPanelNavigationController:(id)arg1 willPresentEngagementPanelRootNavigationViewAndRetainHeight:(_Bool)arg2;
- (id)activeVideoReportingRenderer;
- (_Bool)isShowingLiveChat;
- (void)showLiveChat:(_Bool)arg1;
- (void)showAddToMenuFromView:(id)arg1;
- (void)adDidComplete:(long long)arg1;
- (void)hideWatch;
- (void)showSmallScreenWithCompletion:(CDUnknownBlockType)arg1;
- (void)showSmallScreen;
- (void)showFullScreen;
- (void)playbackControllerContentOverlayDidChange:(id)arg1;
- (void)playbackControllerExternalPlaybackActiveStateDidChange:(_Bool)arg1;
- (void)playbackControllerVRStateDidChange:(_Bool)arg1;
- (void)playbackControllerDidPressPause;
- (void)playbackControllerDidPressReplay;
- (void)playbackControllerDidPressPlay;
- (void)playbackRouteDidChange;
- (void)playbackControllerCurrentVideoAspectRatioDidChange:(double)arg1;
- (void)playbackControllerDidPressCopyDebugInfo;
- (void)playbackControllerDidFinishPlayingAd;
- (void)playbackControllerDidReceivePlayerResponse:(id)arg1;
- (void)playbackControllerDidFailWithError:(id)arg1;
- (void)playbackControllerWillFailWithError:(id)arg1;
- (void)playbackControllerPlayerStateDidChange;
- (void)playbackControllerDidLoadPlayerWithPlaybackData:(id)arg1;
- (void)playbackControllerDidPause;
- (void)playbackControllerDidPlay;
- (void)playbackControllerStateDidChange;
- (void)maybeShowOfflineToast:(id)arg1;
- (void)playbackControllerDidActivateVideo:(id)arg1 withPlaybackData:(id)arg2;
- (void)playbackControllerWillActivateVideo:(id)arg1;
- (void)playbackControllerDidFinishPlayback;
- (void)playbackControllerCurrentVideoMediaTimeDidChange:(double)arg1;
- (id)playbackDebugData;
- (void)didScheduleDeferredRemoveWithVideoID:(id)arg1;
- (void)didDeleteOfflineVideoWithID:(id)arg1 pinTypes:(int)arg2;
- (void)didDeleteAllOfflineVideosWithIDs:(id)arg1;
- (id)activeVideoNavEndpoint;
- (_Bool)isActiveVideoPlayable;
- (unsigned long long)activeVideoIndex;
- (id)activePlaylistID;
- (double)activeVideoMediaTime;
- (id)activeVideoID;
- (void)setPlaylistPanelController:(id)arg1;
@property(readonly, nonatomic) UIView *descriptionDetailsView;
@property(readonly, nonatomic) UIView *playerView;
@property(readonly, nonatomic) UIView *watchNextView;
- (_Bool)shouldReloadVideoForWatchTransition:(id)arg1;
- (void)handleRemoteControlEvent:(long long)arg1;
- (long long)handleChangePlaybackPositionCommand:(id)arg1;
- (long long)handleNextTrackCommand:(id)arg1;
- (long long)handlePreviousTrackCommand:(id)arg1;
- (long long)handlePauseCommand:(id)arg1;
- (long long)handlePlayCommand:(id)arg1;
- (long long)handleTogglePlayPauseCommand:(id)arg1;
- (long long)noopHandler:(id)arg1;
- (void)clearNavEndpoint;
- (void)reloadWatchNextIfNecessary;
- (void)reloadStartPlayback:(_Bool)arg1;
- (void)reload;
- (void)setWatchTransition:(id)arg1 startPlayback:(_Bool)arg2;
- (void)setWatchTransition:(id)arg1;
- (void)prepareForWatchTransition:(id)arg1;
- (id)parentCSN;
- (void)dealloc;
- (id)initWithWatchFlowController:(id)arg1 parentResponder:(id)arg2 handleRemoteCommands:(_Bool)arg3;
- (id)initWithWatchFlowController:(id)arg1 parentResponder:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

