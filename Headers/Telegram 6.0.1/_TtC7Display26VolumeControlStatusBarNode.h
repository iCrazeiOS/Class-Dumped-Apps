//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AsyncDisplayKit/ASDisplayNode.h>

@interface _TtC7Display26VolumeControlStatusBarNode : ASDisplayNode
{
    // Error parsing type: , name: innerGraphics
    // Error parsing type: , name: graphics
    // Error parsing type: , name: outlineNode
    // Error parsing type: , name: backgroundNode
    // Error parsing type: , name: iconNode
    // Error parsing type: , name: foregroundNode
    // Error parsing type: , name: foregroundClippingNode
    // Error parsing type: , name: validLayout
    // Error parsing type: , name: isDark
    // Error parsing type: , name: value
}

- (void).cxx_destruct;
- (id)initWithLayerBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)initWithViewBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)init;

@end

