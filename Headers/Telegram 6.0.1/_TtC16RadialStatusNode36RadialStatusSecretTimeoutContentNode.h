//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TelegramUI/_TtC16RadialStatusNode23RadialStatusContentNode.h>

@interface _TtC16RadialStatusNode36RadialStatusSecretTimeoutContentNode : _TtC16RadialStatusNode23RadialStatusContentNode
{
    // Error parsing type: , name: color
    // Error parsing type: , name: beginTime
    // Error parsing type: , name: timeout
    // Error parsing type: , name: icon
    // Error parsing type: , name: sparks
    // Error parsing type: , name: progress
    // Error parsing type: , name: particles
    // Error parsing type: , name: displayLink
}

+ (void)drawRect:(struct CGRect)arg1 withParameters:(id)arg2 isCancelled:(CDUnknownBlockType)arg3 isRasterizing:(_Bool)arg4;
- (void).cxx_destruct;
- (id)initWithLayerBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)initWithViewBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)init;
- (id)drawParametersForAsyncLayer:(id)arg1;
- (void)didExitHierarchy;
- (void)willEnterHierarchy;
- (void)layout;
- (void)dealloc;

@end

