//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AsyncDisplayKit/ASDisplayNode.h>

@interface _TtC10TelegramUI36ChatTextInputSlowmodePlaceholderNode : ASDisplayNode
{
    // Error parsing type: , name: theme
    // Error parsing type: , name: iconNode
    // Error parsing type: , name: iconArrowContainerNode
    // Error parsing type: , name: iconArrowNode
    // Error parsing type: , name: textNode
    // Error parsing type: , name: slowmodeState
    // Error parsing type: , name: validLayout
    // Error parsing type: , name: timer
}

- (void).cxx_destruct;
- (id)initWithLayerBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)initWithViewBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)init;
- (void)dealloc;

@end

