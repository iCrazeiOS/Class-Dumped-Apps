//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AsyncDisplayKit/ASDisplayNode.h>

@interface _TtC20ManagedAnimationNode20ManagedAnimationNode : ASDisplayNode
{
    // Error parsing type: , name: intrinsicSize
    // Error parsing type: , name: imageNode
    // Error parsing type: , name: displayLink
    // Error parsing type: , name: state
    // Error parsing type: , name: trackStack
    // Error parsing type: , name: didTryAdvancingState
}

- (void).cxx_destruct;
- (id)initWithLayerBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)initWithViewBlock:(CDUnknownBlockType)arg1 didLoadBlock:(CDUnknownBlockType)arg2;
- (id)init;

@end

