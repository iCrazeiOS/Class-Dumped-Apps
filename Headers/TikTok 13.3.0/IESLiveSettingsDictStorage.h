//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "IESLiveSettingsDictStorage-Protocol.h"

@class NSDictionary, NSString;
@protocol HTSKVStore;

@interface IESLiveSettingsDictStorage : NSObject <IESLiveSettingsDictStorage>
{
    _Bool _fetched;
    NSDictionary *_dict;
    NSString *_settingsKey;
    id <HTSKVStore> _kvStore;
}

@property(retain, nonatomic) id <HTSKVStore> kvStore; // @synthesize kvStore=_kvStore;
@property(copy, nonatomic) NSString *settingsKey; // @synthesize settingsKey=_settingsKey;
@property _Bool fetched; // @synthesize fetched=_fetched;
- (void).cxx_destruct;
- (void)saveData:(id)arg1 toKey:(id)arg2;
@property(copy, nonatomic) NSDictionary *dict; // @synthesize dict=_dict;
- (id)initWithSettingsKey:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

