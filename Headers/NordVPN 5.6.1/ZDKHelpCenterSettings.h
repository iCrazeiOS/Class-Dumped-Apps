//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSString;

@interface ZDKHelpCenterSettings : NSObject
{
    // Error parsing type: , name: helpCenterArticleVotingEnabled
    // Error parsing type: , name: enabled
    // Error parsing type: , name: locale
}

- (void).cxx_destruct;
- (id)init;
@property(nonatomic, readonly) NSString *locale;
@property(nonatomic, readonly) _Bool enabled; // @synthesize enabled;
@property(nonatomic, readonly) _Bool helpCenterArticleVotingEnabled; // @synthesize helpCenterArticleVotingEnabled;

@end

